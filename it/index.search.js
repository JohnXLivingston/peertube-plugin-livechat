...
Suggerimento You can test the livechat plugin with this demo page.
...
You can even activate the chat for specific VOD videos. This is how the demo page works: it is not a live stream, but I have activated the chat specifically for this video.`,description:"Introduction",tags:[],title:"Introduction",uri:"/peertube-plugin-livechat/it/intro/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:` Suggerimento This Code of Conduct is adapted from the Contributor Covenant, version 2.1, available at https://www.contributor-covenant.org/version/2/1/code_of_conduct.html. Translations are available at https://www.contributor-covenant.org/translations. Instances of abusive, harassing, or otherwise unacceptable behavior may be reported to the community leaders responsible for enforcement by mail at git.[at].john-livingston.fr.
...
For answers to common questions about this code of conduct, see the FAQ at https://www.contributor-covenant.org/faq. Translations are available at https://www.contributor-covenant.org/translations.`,description:"Codice di comportamento dei Contributori⋅trici",tags:[],title:"Code of Conduct",uri:"/peertube-plugin-livechat/it/contributing/codeofconduct/index.html"},{breadcrumb:"Peertube livechat",content:` User documentationPlugin peertube-plugin-livechat user documentation
...
If you haven’t a Peertube account Attenzione This feature can be disabled by the instance’s adminitrators.
...
Log in using an external authentication provider Attenzione This feature can be disabled by the instance’s adminitrators.
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/it/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`You can configure some words that will be automatically moderated by the bot (messages containing such words will be instantly deleted). You can also add an optional reason that will be displayed at the place of deleted messages. Several examples are provided on the documentation page. You can fill several “Forbidden words or expressions” fields. When a user sends a message that match the configured criteria, the message will automatically be deleted.
...
Suggerimento These words are case insensitive.
...
Also moderate messages from moderators By default, moderator messages will not be deleted when containing forbidden words. By checking this option, messages from moderators will also be deleted. Reason Reason to display besides deleted messages Comments You can add here some comments about this rule, to remember how and why you created it. These comments are purely indicative, and have no influence on the bot's behavior. `,description:"The bot can automatically moderate messages containing forbidden words.",tags:[],title:"Forbidden words",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/bot/forbidden_words/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation",content:`This section describes the plugin settings page.
...
Informazione Changing this setting will restart the chat server, and all users will be disconnected for a short time.
...
Non visualizzare le chat remote Selezionando questa impostazione la vostra istanza non visualizzerà mai chat da video remoti. Non pubblicare le informazioni sulla chat Selezionando questo parametro, la vostra istanza non pubblicherà le informazioni della chat sul fediverso. Le istanze Peertube remote non sapranno che una chat è associata ai video.
...
Attenzione: questo parametro influisce solo sulla pubblicazione di informazioni tramite il protocollo ActivityPub. Non impedisce a un'applicazione remota di rilevare in altro modo la presenza della chat e di tentare di connettersi ad essa. Authentication Disable livechat tokens In case you have any trouble with the long term authentication tokens, you can disable the feature here.
...
Chat behaviour Tipo di chat room Puoi scegliere di avere delle chat room separate per ogni video o di raggrupparle per canale. Apri automaticamente la chat Quando si guarda un video, la chat si aprirà automaticamente. Mostra il pulsante “Apri in una nuova finestra” Ci sarà un pulsante per aprire la chat in una nuova finestra. Mostra il pulsante “Condividi il link della chat room” This feature enables a «share chat link» modal. With this modal, you can generate URLs to join the chat. The chat can be customized (readonly mode, use the current theme, …).
...
Lз utentə possono attivare la chat per i loro video in diretta Se selezionata, tutti i video in diretta avranno una casella di controllo nelle loro proprietà per attivare/disattivare la chat. The video owner will be able to activate web chats.
Attiva la chat per tutti i video in diretta Se selezionata, la chat sarà attivata per tutti i video in diretta. Attiva la chat per tutti i video che non sono dirette Attiva la chat per tutti i video che non sono dirette Attiva la chat per questi video Attiva la chat per questi video Nascondi la chat per gli utenti anonimi (non connessi) Se selezionata, gli utenti anonimi (non connessi) non vedranno la chat. Note: for now this feature simply hide the chat. In a future release, the chat will be replaced by a message saying «please log in to […]». See v5.7.0 Release Notes for more information.
Ban anonymous user's IP when user is banned from a chatroom By enabling this option, each time an anonymous user is banned from a chatroom, it's IP will also be banned from the chat server. Warning: if your instance is open to registration, any user could create a trapped-room, invite users to join, and automatically ban all anonymous user's IPs. The banned IP list is not stored, it will be cleared on server restart, or when you change some plugin's settings. The banned IP are logged in the Prosody server log files, so server's administrators can eventually use some external tools (like fail2ban) to ban IPs more widely. Important note: If you enable this feature, and are using a custom reverse proxy on front of Peertube, please make sure that your setup is correctly configured to forward real user’s IPs to Peertube. Otherwise it could block all anonymous users at once.
...
Peertube theme: this is a special theme, made especially for peertube’s integration. Default ConverseJS theme: this is the default ConverseJS theme. ConverseJS concord theme: this is a theme provided by ConverseJS. Rilevamento automatico dei colori Cerca di rilevare automaticamente i colori dal tema corrente dell'utente.
...
Se questo non funzionasse correttamente per alcuni temi di Peertube, disattiva questa opzione. Puoi segnalare i bug riscontrati nella gestione errori ufficiale . Non dimenticare di indicare il tema difettoso. Attributi di stile dell'iframe della chat Stili aggiuntivi da applicare all'iframe della chat.
Esempio: height:400px; Chat server advanced settings Usa il server Prosody installato sul sistema The plugin comes with an AppImage that is used to run the Prosody XMPP server. If this AppImage is not working, you can fallback to the Prosody that is packaged for your server. Just install the prosody package.
This setting should only be used if the plugin is broken, and waiting for a patch.
Disattiva Websocket Con Peertube >= 5.0.0, questo plugin cercherà di utilizzare il protocollo Websocket per le connessioni alla chat. Se il browser o la connessione dell'utente non sono compatibili, il browser passerà automaticamente al protocollo BOSH. Tuttavia, in rari casi, questo potrebbe non funzionare. Ad esempio, se hai un reverse proxy di fronte a Peertube che non consente le connessioni tramite Websocket. In tal caso, puoi selezionare quest'opzione per disattivare le connessioni Websocket. Porta per Prosody La porta che sarà utilizzata dal server Prosody.
...
Nota: questo potrebbe cambiare prossimamente, poiché è prevista l'aggiunta di parametri per attivare le connessioni esterne. URL di Peertube per le chiamate API Ti preghiamo di non toccare questa impostazione se non sei sicurə di quello che stai facendo.
In rari casi il server Prosody potrebbe non essere in grado di chiamare l'API di Peertube utilizzando il suo url pubblico. Puoi allora utilizzare questo parametro per personalizzare l'url che i moduli Prosody utilizzeranno per le API di Peertube (ad esempio «http://localhost:9000» o «http://127.0.0.1:9000»). If this setting is left empty, and you are using Peertube >= 5.1 or later, the plugin will use values from your Peertube configuration file to guess on which interface and port request have to be done.
...
Si dovrebbe usare la porta standard 5269. Altrimenti sarà necessario impostare un record DNS specifico . Interfacce di rete da server a server Le interfacce di rete su cui ascoltare per le connessioni da server a server.
...
Esempi: *, :: * 127.0.0.1, ::1 172.18.0.42 Cartella dei certificati Se questo campo è vuoto, il plugin genererà e utilizzerà certificati autofirmati.
Se si desidera utilizzare altri certificati, è sufficiente specificare la cartella in cui Prosody può trovarli. Nota: l'utente "peertube" deve avere accesso in lettura a questa cartella. Abilitare le connessioni dal client al server Consenti ai clienti XMPP a connettersi al server Prosody incorporato nel plugin.
Questa opzione consente unicamente le connessioni dei clienti sul localhost. This setting enable XMPP clients to connect to the built-in Prosody server. For now, this option only allows connections from localhost clients.
...
Nota: questo potrebbe cambiare prossimamente, poiché è prevista l'aggiunta di una funzione per attivare le connessioni dall'esterno. Client to server network interfaces The network interfaces to listen on for client to server connections.
...
vendor The vendor folder is not part of the source code. It is used during the build process to download some external source code.`,description:"Source code organization",tags:[],title:"Source code",uri:"/peertube-plugin-livechat/it/technical/sourcecode/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`I just installed/upgraded the plugin, but nothing happens If you have just installed/upgraded the plugin, but nothing happens (no chat, no settings, buttons in the settings page does not work, …), just try to reload the page.
...
In such case, try changing the “URL di Peertube per le chiamate API” settings, by setting http://127.0.0.1:9000 (assuming 9000 is the port on which Peertube listen, ask your instance administrators if you don’t know).
...
If you can’t fix this immediatly, you can disable Websocket by unchecking “Disattiva Websocket” in the plugin setting page. In such case, you should also check “Non pubblicare le informazioni sulla chat”, as chat federation won’t work without Websocket.`,description:"Some classic mistakes and workarounds.",tags:[],title:"Troubleshooting",uri:"/peertube-plugin-livechat/it/documentation/installation/troubleshooting/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` For viewersHow to chat for stream viewers
OBSDocumentation to stream the chat content using OBS.
XMPP ClientsConnect to chat using a XMPP client
For streamersHow to setup the chat for your live stream
Some basicsSome basics about how to setup and use the chat for your live stream
...
OpenID Connect Attenzione This feature is still experimental. This feature is available with the plugin version >= 9.0.0.
...
More to come Other authentication methods will be implemented in the future.`,description:"Plugin Peertube Livechat settings - External Authentication",tags:[],title:"External Authentication",uri:"/peertube-plugin-livechat/it/documentation/admin/external_auth/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` Informazione Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
Suggerimento To install or update the plugin, just use the Peertube web admin interface.
Here are some other more specific instructions:
TroubleshootingSome classic mistakes and workarounds.
Known issues: CPU CompatibilityFor now, the plugin only works out of the box for x86_64 and arm64 CPU architecture. Here are some instructions for other CPU architectures.
Upgrade from version older than 6.0.0Important notes when upgrading for an older version.`,description:"Plugin peertube-plugin-livechat installation guide",tags:[],title:"Installation guide",uri:"/peertube-plugin-livechat/it/documentation/installation/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`The Prosody AppImage included in the plugin will only work on x86_64 and arm64 CPU. It is not compatible with other CPU architectures.
...
opening a web page (with an url like https://yourinstance.tld/plugins/livechat/router/webchat/room/8df24108-6e70-4fc8-b1cc-f2db7fcdd535), using a XMPP client (and joining a room like xmpp://8df24108-6e70-4fc8-b1cc-f2db7fcdd535@room.yourinstance.tld?join) Attenzione Joining the chat using a XMPP client is not always possible. It requires some DNS and server configuration. It will only be possible if instance’s admins have correctly setup the external XMPP clients connection feature.
Attenzione Don’t try to gues these url and connection methods yourself. Please report to next chapters.
...
Informazione This requires Peertube >= 5.1
This follows the FEP-1970 recommendations.
Attenzione At the time of the writing, this FEP is in draft status, and the livechat plugin is a Proof-of-concept. Until the FEP is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Informazione This requires Peertube >= 5.2
Informazione The <podcast:chat> element is currently only supported for live streams.
This follows the <podcast:chat> proposal.
Attenzione At the time of the writing, this proposal is in draft status, and the livechat plugin is a Proof-of-concept. Until the proposal is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Examples Don’t hesitate to share your rules. To do so, you can for example edit this page.`,description:"Advanced firewall rules for the Prosody server",tags:[],title:"Prosody mod_firewall",uri:"/peertube-plugin-livechat/it/documentation/admin/mod_firewall/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation > Advanced usage",content:`This chat module is based on the XMPP protocol, also known as Jabber. It is therefore possible to connect to the chats using XMPP client software. This can for example be useful to facilitate moderation operations.
For the user documentation associated with these features, please refer to the user documentation page.
Informazione Enabling these features requires configuration changes on the server, and on the DNS records. It is not possible to configure this only from the Peertube interface, and it requires some basic system some basic system admin skills.
Login to your Peertube account Attenzione This feature is not yet available, and will come in a future version of the plugin.
...
Attenzione Don’t enable this mode on a production server, neither on a public server. This could cause security issues.
...
OBS Dock Informazione This feature comes with the livechat plugin version 10.1.0.
Attenzione This feature can be disabled by the instance’s adminitrators.
...
To do so, just use the “Condividi il link della chat” feature, and open the “Dock” tab. From there, you can create a new token using the “+” button.
...
Suggerimento Tokens are valid to join any chat room. You don’t have to generate separate tokens for each of your rooms. You can also customize the nickame that will be used by changing the n parameter in the url.
...
Attenzione Some linux distro (like Debian Bullseye for example) have too old version of po4a. Please make sure to install a compatible version. If you are using Debian Bullseye for example, you can download the Bookworm po4a.deb file from https://packages.debian.org, and install it manually.
...
Attenzione There may be links to documentation elsewhere on the web. Try not to change the urls of the documentation pages. Or at the very least, put links to the new location on the previous url.
...
Internal connections only (basic) You will need to enable Abilitare le connessioni dal client al server in the livechat plugin settings.
...
Informazione This documentation use an anonymous account to connect the bridge to the chat. But since the livechat v10.1.0, there is a new way to generate long term authentication token, that allows to connect using your account. This is used for OBS docks. Using this feature for other purposes is not documented and not officially supported yet. If you want to use it anyway, you can request a token by calling then /plugins/livechat/router/api/auth/tokens endpoint. To get needed headers and request body, just check what happens when you generate a new token for OBS docks.`,description:"Using Matterbridge to bridge with other chats",tags:[],title:"Using Matterbridge",uri:"/peertube-plugin-livechat/it/documentation/admin/advanced/matterbridge/index.html"},{breadcrumb:"Peertube livechat",content:`Interested in contributing? Awesome!
Code of ConductCodice di comportamento dei Contributori⋅trici
TranslateTranslate the plugin
Give your feedbackGive your feedback
DevelopDevelop
DocumentationDocument the plugin, or translate the documentation.`,description:"Contributing",tags:[],title:"Contributing",uri:"/peertube-plugin-livechat/it/contributing/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The livechat plugin includes a “slow mode” feature, to rate limit the number of messages that a user can send to a given MUC room. At time of writing, there were no XEP to describe such feature. Please find below a XEP draft, that will be submitted for review.
Attenzione Work In Progress, this page is not done yet. For an updated version of this document, you can check the draft XEP XMP file.
...
Appendix F: Requirements Conformance The following requirements keywords as used in this document are to be interpreted as described in RFC 2119: “MUST”, “SHALL”, “REQUIRED”; “MUST NOT”, “SHALL NOT”; “SHOULD”, “RECOMMENDED”; “SHOULD NOT”, “NOT RECOMMENDED”; “MAY”, “OPTIONAL”.`,description:"MUC Slow mode XEP",tags:[],title:"MUC Slow mode",uri:"/peertube-plugin-livechat/it/technical/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:`This chat plugin relies on the XMPP protocol (also known as Jabber). It is therefore possible to connect to the chats using XMPP client software. This can be useful for example to facilitate moderation operations.
Informazione The features described on this page must be enabled and configured by your Peertube instance’s administrators. You may therefore not have access to them.
Login to your Peertube account Attenzione This feature is not yet available, and will come in a future version of the plugin.
...
Informazione By default, the share button is only visible to the owner of the video, and the admins/moderators of the instance. However, admins can decide to display this button for everyone.
Then, choose “Connect using XMPP”:
Then you just have to click on “open” or copy/paste the address of the chat room into your XMPP client (using the “join a room” feature).`,description:"Connect to chat using a XMPP client",tags:[],title:"XMPP Clients",uri:"/peertube-plugin-livechat/it/documentation/user/xmpp_clients/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The poll system relies on two thinks:
...
Moderator notes overviewModerator Notes Application technical overview`,description:"Technical documentation",tags:[],title:"Technical documentation",uri:"/peertube-plugin-livechat/it/technical/index.html"},{breadcrumb:"Peertube livechat",content:`Se avete domande, o volete parlare di questo plugin, potete unirvi a questa stanza XMPP con qualsiasi client Jabber: plugin-livechat-support@room.im.yiny.org.
Se volete sostenere il progetto finanziariamente, potete contattarmi via email all’indirizzo git.[at].john-livingston.fr. o utilizzare il mio profilo Liberapay.`,description:"Contattare l'autore",tags:[],title:"Contattarmi",uri:"/peertube-plugin-livechat/it/contact/index.html"},{breadcrumb:"Peertube livechat",content:`package.json, COPYRIGHT and LICENSE files contain the license information for this software and its dependencies.
...
And thanks to all individual contributors who have made a donation via my liberapay page.`,description:"Plugin Credits",tags:[],title:"Credits",uri:"/peertube-plugin-livechat/it/credits/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:`Enabling the chat for you live streams Attenzione Instance administrators can choose to disable or enable chat in specific cases. Information in this section are only true in the default case.
When you create or modify a Peertube live, there is a “plugin settings” tab:
In the “plugin settings” tab, there is a “Attiva la chat” checkbox. Just check or uncheck it to enable or disable the chat associated to your video.
Suggerimento There can be other settings in this tab, depending on plugins installed on your Peertube instance.
...
Share the chat On top of the chat, there is a “Condividi il link della chat” button.
...
In sola lettura: you will only be able to read the chat, not write. This is useful to include the chat content in your live stream (see the OBS documentation). Utilizza i colori del tema corrente: if checked, your current theme colors will be added to the url, so that any user that opens the link will have the same color set. Genera un iframe per incorporare la chat in un sito web: instead of an url, you will obtain an HTML snippet that you can add to your website to embed the chat. For more information on the “Dock” tab, check the OBS documentation.
...
The “Condividi il link della chat” popup can also contain a “Connessione con un client XMPP” tab. This will only be available if your instance’s administators have enabled an correctly configured this option. Using this option, you can provide a link to join the chat using any XMPP client software. Using such softwares can for example facilitate moderation actions.
...
The chat will be automatically recreated each time someone tries to join it as long as the video exists, and has the “Attiva la chat” feature activated.`,description:"Some basics about how to setup and use the chat for your live stream",tags:[],title:"Some basics",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/basics/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Informazione This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
Forbidden wordsThe bot can automatically moderate messages containing forbidden words.
TimersThe bot can send periodically some messages.
CommandsThe bot can respond to several commands.`,description:"How to setup the chat for your live stream",tags:[],title:"For streamers",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Attenzione This section is still incomplete.
Attenzione This page describes the behaviour of livechat versions >= 10.0.0. There were some changes in the way we manage access rights for Peertube administrators and moderators.
...
Suggerimento The video owner will be owner of the chat room. This means they can configure the room, delete it, promote other users as admins, …
Suggerimento Starting with livechat v10.0.0, Peertube instance’s admins and moderators have no special rights on rooms by default. However, they have a special button available on top of the chat: “Become moderator”. Clicking this button will give them owner access on the room.
...
Mute anonymous users Informazione This feature comes with the livechat plugin version 10.2.0.
...
Attenzione This section is still incomplete.
You can promote users as moderators, if you need some help.
Anonymize moderation actions Informazione This feature comes with the livechat plugin version 11.0.0.
...
You can choose to enable or disable this feature for new chatrooms on the channel configuration page.
Participant message history search Informazione This feature comes with the livechat plugin version 11.0.0.
As a room admin or owner, you can search all messages sent by a given participant.
To do so, you have several ways:
using the “Search all messages” action in the dropdown menu besides participants in the sidebar using the “Search all messages” action in the dropdown menu besides chat messages Suggerimento To have more space and better readability, open the chat in full-page mode.
...
From there, you can also promote yourself as room moderator by using the “Become moderator” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Informazione This feature comes with the livechat plugin version 10.2.0.
...
Informazione Peertube instance’s admin can also set global terms & conditions. If so, these terms will be shown above your channel’s terms.
Informazione Anonymous users will only see the terms & conditions once they have chosen their nickname (in other words: once they are able to talk).
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Informazione This feature comes with the livechat plugin version 10.3.0.
...
Attenzione Currently, this feature has one known bug: users that join the chat will get all messages, even messages that are still pending for other participants. However, messages sent after they joined will be delayed correctly.
Suggerimento You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Informazione This feature comes with the livechat plugin version 10.1.0.
...
Suggerimento Anonymous viewers can only vote once they have choosen their nickname.
...
Informazione For viewers using XMPP clients or outdated livechat plugin versions, the banner will not be visible. But they will see the message in the chat and will be able to vote by sending messages with their choices.
When the poll ends, a new message will be sent in the chat, with the results.
Informazione The only way to get old polls results is to search for the poll end message in the chat. For now, polls results are not saved by any other means. So don’t forget to note polls results if you want to keep them.`,description:"You can create polls to ask viewers their opinion.",tags:[],title:"Polls",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/polls/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Informazione This feature comes with the livechat plugin version 10.0.0.
...
Suggerimento To have more space and better readability, open the chat in full-page mode.
...
Suggerimento All modification are instantly visible in all your browser tabs, and for all room’s admins.
...
Attenzione Currently the video vs channel rooms is an instance-wide settings. Only Peertube admins can change it, and it applies to all chatrooms. In the future, this choice will be added in your channel’s options.
...
Sorting notes You can sort notes simply using drag & drop.`,description:"Plugin peertube-plugin-livechat moderation notes",tags:[],title:"Moderation notes",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/moderation_notes/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Informazione This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
The bot will reload instantly when you save the page.`,description:"Chat bot setup",tags:[],title:"Chat bot",uri:"/peertube-plugin-livechat/it/documentation/user/streamers/bot/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Categorie",uri:"/peertube-plugin-livechat/it/categories/index.html"},{breadcrumb:"",content:`PeerTube plugin livechat Suggerimento You can use the language selector in the left menu to view this documentation in different languages. Some translations are missing or incomplete. In this case, you’ll see the English version of the text.
...
Suggerimento You can use the searchbox in the left menu to quickly find specific documentation parts.
Informazione Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
...
ContattarmiContattare l'autore
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/it/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Tag",uri:"/peertube-plugin-livechat/it/tags/index.html"}]