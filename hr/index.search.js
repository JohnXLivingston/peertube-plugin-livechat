...
Konfiguracija kanalaPeertube channel chatrooms configuration
...
Dokumentacija za administratorePlugin Peertube Livechat administration
PostavkePlugin Peertube Livechat settings
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/hr/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za korisnike > For streamers > Chat bot",content:`Možeš konfigurirati neke riječi koje će bot automatski moderirati (poruke koje sadrže takve riječi će se odmah izbrisati). Možeš dodati i opcionalni razlog koji će se prikazati na mjestu izbrisanih poruka. Stranica dokumentacije sadrži nekoliko primjera. You can fill several “Zabranjene riječi i izrazi” fields. When a user sends a message that match the configured criteria, the message will automatically be deleted.
Zabranjene riječi i izrazi In this field, you can set several words, group of words, or “regular expressions”.
Jedna riječ ili izraz po retku. Ako upišeš više riječi u jedan redak, pronaći će samo poruke koje sadrže cijeli niz. Each time a user sends a message, these words will be tested. If the message containes one of them, the message will be deleted.
...
For answers to common questions about this code of conduct, see the FAQ at https://www.contributor-covenant.org/faq. Translations are available at https://www.contributor-covenant.org/translations.`,description:"Contributor Covenant Code of Conduct",tags:[],title:"Kodeks ponašanja",uri:"/peertube-plugin-livechat/hr/contributing/codeofconduct/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za administratore",content:`This section describes the plugin settings page.
Uvjeti i odredbe kanala chata Možeš konfigurirati poruku „uvjeti i odredbe” koja će se prikazati korisnicima koji se pridružuju tvojim chat sobama. For more information on this feature, check the documentation for channel’s terms & conditions.
...
Nemoj prikazati chatove drugih instanci Označavanjem ove postavke tvoja instanca nikada neće prikazati chatove od videa drugih instanci. Nemoj objaviti podatke chata By checking this setting, your instance will not publish chat information on the fediverse. Remote Peertube instances will not be aware that they are chat rooms associated to your videos.
...
Please note: this setting only affects the publication of information via the ActivityPub protocol. It will not prevent a remote application from otherwise detecting the presence of chats, and trying to connect to it. Autentifikacija Deaktiviraj tokene za chat uživo In case you have any trouble with the long term authentication tokens, you can disable the feature here.
...
Chat behaviour Vrsta sobe Ovdje možeš odabrati želiš li zasebne sobe za svaki video ili ih grupirati po kanalu. Automatski otvori chat Prilikom gledanja videa automatski će se otvoriti prozor za chat. Prikaži gumb „Otvori u novom prozoru” U novom prozoru će se pojavit gumb za otvaranje web chata. Prikaži gumb „Dijeli poveznicu chata” This feature enables a «share chat link» modal. With this modal, you can generate URLs to join the chat. The chat can be customized (readonly mode, use the current theme, …).
...
Korisnici mogu aktivirati chat za svoje prijenose uživo Ako je označeno, sva videa uživo imat će potvrdni okvir u svojim svojstvima za aktiviranje web chata. The video owner will be able to activate web chats.
Aktiviraj chat za sve prijenose uživo Ako je označeno, chat će se aktivirati za sve prijenose uživo. Aktiviraj chat za sve prijenose koji nisu uživo Aktiviraj chat za sve prijenose koji nisu uživo Aktiviraj chat za ova videa Aktiviraj chat za ova videa Sakrij chat za anonimne korisnike If checked, anonymous Peertube users won't see the chat. This feature is still experimental. If you enabled it, it is highly recommended to also check "Don't publish chat information". Otherwise, some third party tools could try to open the chat, and have unpredictable behaviours. Note: for now this feature simply hide the chat. In a future release, the chat will be replaced by a message saying «please log in to […]». See v5.7.0 Release Notes for more information.
...
Theming Skup avatara You can choose from several different sets the default avatars that will be used for chat users.
...
Peertube theme: this is a special theme, made especially for peertube’s integration. Default ConverseJS theme: this is the default ConverseJS theme. ConverseJS concord theme: this is a theme provided by ConverseJS. Automatsko prepoznavanje boja Try to autodetect colors from user's current theme.
When this setting is enabled, the plugin tries to auto-detect colors to apply to the chat theme.
If this is not correctly working for some of your Peertube theme, you can disable this option. You can report the bug on the official issue tracker . Don't forget to specify which theme is not working. Atribut stila ifreama za chat Dodatni stilovi koji se dodaju atributu stila iframea. Primjer: height:400px; Napredne postavke chat servera Koristi Prosody sustav The plugin comes with an AppImage that is used to run the Prosody XMPP server. If this AppImage is not working, you can fallback to the Prosody that is packaged for your server. Just install the prosody package.
This setting should only be used if the plugin is broken, and waiting for a patch.
Onemogući Websocket With Peertube >= 5.0.0, this plugin try to use Websocket connection for chatting. If the user's browser or connection is incompatible, the browser will automatically fall back on the BOSH protocol. But in rare case, this can fail. For example, if you have a reverse proxy in front of Peertube that does not allow Websocket connection for plugins. In this case, you can check this setting to disable Websocket connections. Prosody priključak The port that will be used by the Prosody server.
Change it if this port is already in use on your server.
You can close this port on your firewall, it will not be accessed from the outer world.
Note: this might change in a near future, as it is planned to add a feature to activate external connections. Peertube URL za API pozive Please let this setting empty if you don't know what you are doing.
In some rare cases, Prosody can't call Peertube's API from its public URI. You can use this field to customize Peertube's URI for Prosody modules (for example, with «http://localhost:9000» or «http://127.0.0.1:9000»). Ako je ova postavka ostavljena prazna i koristiš Peertube verziju ≥ 5.1 ili noviju, dodatak će koristiti vrijednosti iz tvoje Peertube konfiguracijske datoteke kako bi pogodio na kojem sučelju i priključku treba izvršiti zahtjev.
...
Arhiviranje sadržaja se uvijek može aktivirati/deaktivirati za određenu sobu, uređivanjem njezinih svojstava. Istek dnevnika soba Možeš odabrati koliko dugo server čuva sadržaj chat sobe. Vrijednost može biti: 60: sadržaj će se čuvati 60 sekundi. 60 možeš zamijeniti s bilo kojim cijelim brojem. 1d: sadržaj će se čuvati 1 dan. 1 možeš zamijeniti s bilo kojim cijelim brojem. 1w: sadržaj će se čuvati 1 tjedan. 1 možeš zamijeniti s bilo kojim cijelim brojem. 1m: sadržaj će se čuvati 1 mjesec. 1 možeš zamijeniti s bilo kojim cijelim brojem. 1y: sadržaj će se čuvati 1 godinu. 1 možeš zamijeniti s bilo kojim cijelim brojem. nikada: sadržaj nikada neće isteći i čuvat će se zauvijek. Omogući vezu sa sobom pomoću eksternih XMPP računa By enabling this option, it will be possible to connect to rooms using external XMPP accounts and XMPP clients.
...
You should use the standard 5269 port. Otherwise you should setup a specific DNS record . Mrežna sučelja za vezu između servera i servera The network interfaces to listen on for server to server connections.
...
Examples: *, :: * 127.0.0.1, ::1 172.18.0.42 Mapa certifikata Ako je ovo polje prazno, dodatak će generirati i koristiti samopotpisane certifikate.
Ako želiš koristiti druge certifikate, ovdje navedi ovdje mapu gdje ih Prosody može pronaći. Napomena: „peertube” korisnik mora imati dozvolu za čitanje ove mape. Omogući veze između klijenta i servera Enable XMPP clients to connect to the built-in Prosody server.
...
Note: this might change in a near future, as it is planned to add a feature to activate external connections. Mrežna sučelja za vezu između klijenta i servera The network interfaces to listen on for client to server connections.
...
vendor The vendor folder is not part of the source code. It is used during the build process to download some external source code.`,description:"Source code organization",tags:[],title:"Source code",uri:"/peertube-plugin-livechat/hr/technical/sourcecode/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Installation guide",content:`I just installed/upgraded the plugin, but nothing happens If you have just installed/upgraded the plugin, but nothing happens (no chat, no settings, buttons in the settings page does not work, …), just try to reload the page.
...
Oznaka za gumb povezivanja Ova će se oznaka prikazati korisnicima kao oznaka gumba za autentifikaciju kod ovog pružatelja usluge OpenID Connect. This is the button label in the following screenshot:
...
TroubleshootingSome classic mistakes and workarounds.
Known issues: CPU CompatibilityFor now, the plugin only works out of the box for x86_64 and arm64 CPU architecture. Here are some instructions for other CPU architectures.
Upgrade from version older than 6.0.0Important notes when upgrading for an older version.`,description:"Plugin peertube-plugin-livechat installation guide",tags:[],title:"Installation guide",uri:"/peertube-plugin-livechat/hr/documentation/installation/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Installation guide",content:`The Prosody AppImage included in the plugin will only work on x86_64 and arm64 CPU. It is not compatible with other CPU architectures.
...
Dodavanje novog narječja If you think there is a missing locale, please check first if it is handled in Peertube. If so, you can open an issue to ask for it.
...
get the Podcast RSS feed for the channel, if there is no <podcast:liveItem> element under the <channel>, stop. find the <podcast:liveItem> you are looking for <podcast:socialInteract> can be used to cross-reference the items with ActivityPub loop through the <podcast:chat> values (if <podcast:chat> is not an array, just iterate on this single value) there should only be one, but you should expect to handle several just in case search for the first entry protocol === xmpp and a space attribute space should be an XMPP JID for a MUC if found, open this XMPP JID with your client after converting it to a join URI, or connect to the XMPP room at that address `,description:"Displaying the livechat with 3rd party software.",tags:[],title:"Third party",uri:"/peertube-plugin-livechat/hr/technical/thirdparty/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za korisnike > For streamers > Chat bot",content:`Možeš konfigurirati nekoliko timera koji će slati poruke u intervalima. Ove će poruke bot slati svakih X minuta. Na primjer, možeš postaviti bota da šalje neke informacije o sponzoriranju svakih 5 minuta. Tip If there is no user in the chatroom, the bot won’t send any message.
Timer Možeš konfigurirati nekoliko timera koji će slati poruke u intervalima. Ove će poruke bot slati svakih X minuta. Na primjer, možeš postaviti bota da šalje neke informacije o sponzoriranju svakih 5 minuta. Jedna poruka po retku. Ako postoji više poruka, nasumično će odabrati jednu svakih X minuta. Šalji svakih X minuta Bot će slati poruku svakih X minuta. `,description:"The bot can send periodically some messages.",tags:[],title:"Timers",uri:"/peertube-plugin-livechat/hr/documentation/user/streamers/bot/quotes/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija",content:` PostavkePlugin Peertube Livechat settings
External AuthenticationPlugin Peertube Livechat settings - External Authentication
Prosody mod_firewallAdvanced firewall rules for the Prosody server
Advanced usageSome advanced features
XMPP clientsAllow connections using XMPP clients
Using MatterbridgeUsing Matterbridge to bridge with other chats`,description:"Plugin Peertube Livechat administration",tags:[],title:"Dokumentacija za administratore",uri:"/peertube-plugin-livechat/hr/documentation/admin/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za administratore",content:` Info This feature comes with the livechat plugin version 11.0.0.
You can enable mod_firewall on your Prosody server.
...
OBS Dock Info Ova je funkcija dostupna s dodatkom za chat uživo verzije 10.1.0.
...
Warning There may be links to documentation elsewhere on the web. Try not to change the urls of the documentation pages. Or at the very least, put links to the new location on the previous url.
...
Internal connections only (basic) You will need to enable Omogući veze između klijenta i servera in the livechat plugin settings.
...
Isključi zvuk anonimnih korisnika Info Ova je funkcija dostupna s dodatkom za chat uživo verzije 10.2.0.
...
Anonymize moderation actions Info This feature comes with the livechat plugin version 11.0.0.
...
You can choose to enable or disable this feature for new chatrooms on the channel configuration page.
Participant message history search Info This feature comes with the livechat plugin version 11.0.0.
...
From there, you can also promote yourself as room moderator by using the “Postani moderator” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/hr/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za korisnike > For streamers",content:` Info Ova je funkcija dostupna s dodatkom za chat uživo verzije 10.2.0.
Konfiguracija You can add terms & conditions to your channel. These terms will be shown to all users joining the chat.
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/hr/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za korisnike > For streamers",content:` Info Ova je funkcija dostupna s dodatkom za chat uživo verzije 10.3.0.
Uvod As a streamer, you can choose to delay messages in the chat, to let some time to moderators to delete messages before they can even be read by other participants.
...
Možeš konfigurirati prilagođene emojije za svoj kanal. Ovi emojiji će biti dostupni u alatu za biranje emojija. Korisnici ih također mogu koristiti sa svojim kratkim imenom (na primjer tako da napišu ":shortname:"). Emotikone možeš koristiti u chatu koristeći ":shortname:". Kratko ime može započeti i/ili završiti dvotočkom (:) te koristiti samo alfanumeričke znakove, podvlake i crtice. Preporuča se započeti ih dvotočkom, kako bi korisnici mogli koristiti automatsko dovršavanje (upisivanjem ":" i pritiskom tipke tabulatora). Uvoz / Izvoz On the channel configuration page, there are an “Uvezi” and an “Izvezi” button. The “Izvezi” button generates a file than you can then import on another channel.
...
Using this feature, for example, you can ask your moderators to highlight all chat questions, so you can see them at a glance during your livestream, and check them as answered.`,description:"You can handle tasks and task lists with your moderation team.",tags:[],title:"Tasks / To-do lists",uri:"/peertube-plugin-livechat/hr/documentation/user/streamers/tasks/index.html"},{breadcrumb:"Peertube livechat > Dokumentacija > Dokumentacija za korisnike > For streamers",content:` Info This feature comes with the livechat plugin version 11.0.0.
Uvod The livechat plugin includes a Moderator Notes Application: you can write some notes, that could be associated to chat participants. Every room’s admins have access to these notes, so they can edit them collaboratively.
...
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/hr/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Tags",uri:"/peertube-plugin-livechat/hr/tags/index.html"}]