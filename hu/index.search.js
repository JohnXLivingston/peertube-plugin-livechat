...
Tipp You can test the livechat plugin with this demo page.
...
You can even activate the chat for specific VOD videos. This is how the demo page works: it is not a live stream, but I have activated the chat specifically for this video.`,description:"Introduction",tags:[],title:"Introduction",uri:"/peertube-plugin-livechat/hu/intro/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:` Tipp This Code of Conduct is adapted from the Contributor Covenant, version 2.1, available at https://www.contributor-covenant.org/version/2/1/code_of_conduct.html. Translations are available at https://www.contributor-covenant.org/translations. Instances of abusive, harassing, or otherwise unacceptable behavior may be reported to the community leaders responsible for enforcement by mail at git.[at].john-livingston.fr.
...
If you haven’t a Peertube account Figyelem This feature can be disabled by the instance’s adminitrators.
...
Log in using an external authentication provider Figyelem This feature can be disabled by the instance’s adminitrators.
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/hu/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`You can configure some words that will be automatically moderated by the bot (messages containing such words will be instantly deleted). You can also add an optional reason that will be displayed at the place of deleted messages. Several examples are provided on the documentation page. You can fill several “Forbidden words or expressions” fields. When a user sends a message that match the configured criteria, the message will automatically be deleted.
...
Tipp These words are case insensitive.
Tipp You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
Figyelem This features is still experimental. There might be some issues with non-latin alphabets. You can open an issue to report your problems.
...
Infó Changing this setting will restart the chat server, and all users will be disconnected for a short time.
...
vendor The vendor folder is not part of the source code. It is used during the build process to download some external source code.`,description:"Source code organization",tags:[],title:"Source code",uri:"/peertube-plugin-livechat/hu/technical/sourcecode/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`I just installed/upgraded the plugin, but nothing happens If you have just installed/upgraded the plugin, but nothing happens (no chat, no settings, buttons in the settings page does not work, …), just try to reload the page.
...
OpenID Connect Figyelem This feature is still experimental. This feature is available with the plugin version >= 9.0.0.
...
More to come Other authentication methods will be implemented in the future.`,description:"Plugin Peertube Livechat settings - External Authentication",tags:[],title:"External Authentication",uri:"/peertube-plugin-livechat/hu/documentation/admin/external_auth/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` Infó Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
Tipp To install or update the plugin, just use the Peertube web admin interface.
Here are some other more specific instructions:
TroubleshootingSome classic mistakes and workarounds.
Known issues: CPU CompatibilityFor now, the plugin only works out of the box for x86_64 and arm64 CPU architecture. Here are some instructions for other CPU architectures.
Upgrade from version older than 6.0.0Important notes when upgrading for an older version.`,description:"Plugin peertube-plugin-livechat installation guide",tags:[],title:"Installation guide",uri:"/peertube-plugin-livechat/hu/documentation/installation/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`The Prosody AppImage included in the plugin will only work on x86_64 and arm64 CPU. It is not compatible with other CPU architectures.
...
opening a web page (with an url like https://yourinstance.tld/plugins/livechat/router/webchat/room/8df24108-6e70-4fc8-b1cc-f2db7fcdd535), using a XMPP client (and joining a room like xmpp://8df24108-6e70-4fc8-b1cc-f2db7fcdd535@room.yourinstance.tld?join) Figyelem Joining the chat using a XMPP client is not always possible. It requires some DNS and server configuration. It will only be possible if instance’s admins have correctly setup the external XMPP clients connection feature.
Figyelem Don’t try to gues these url and connection methods yourself. Please report to next chapters.
...
Infó This requires Peertube >= 5.1
This follows the FEP-1970 recommendations.
Figyelem At the time of the writing, this FEP is in draft status, and the livechat plugin is a Proof-of-concept. Until the FEP is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Infó This requires Peertube >= 5.2
Infó The <podcast:chat> element is currently only supported for live streams.
This follows the <podcast:chat> proposal.
Figyelem At the time of the writing, this proposal is in draft status, and the livechat plugin is a Proof-of-concept. Until the proposal is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Examples Don’t hesitate to share your rules. To do so, you can for example edit this page.`,description:"Advanced firewall rules for the Prosody server",tags:[],title:"Prosody mod_firewall",uri:"/peertube-plugin-livechat/hu/documentation/admin/mod_firewall/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation > Advanced usage",content:`This chat module is based on the XMPP protocol, also known as Jabber. It is therefore possible to connect to the chats using XMPP client software. This can for example be useful to facilitate moderation operations.
For the user documentation associated with these features, please refer to the user documentation page.
Infó Enabling these features requires configuration changes on the server, and on the DNS records. It is not possible to configure this only from the Peertube interface, and it requires some basic system some basic system admin skills.
Login to your Peertube account Figyelem This feature is not yet available, and will come in a future version of the plugin.
...
Figyelem Don’t enable this mode on a production server, neither on a public server. This could cause security issues.
...
OBS Dock Infó This feature comes with the livechat plugin version 10.1.0.
Figyelem This feature can be disabled by the instance’s adminitrators.
...
Tipp Tokens are valid to join any chat room. You don’t have to generate separate tokens for each of your rooms. You can also customize the nickame that will be used by changing the n parameter in the url.
...
Figyelem Some linux distro (like Debian Bullseye for example) have too old version of po4a. Please make sure to install a compatible version. If you are using Debian Bullseye for example, you can download the Bookworm po4a.deb file from https://packages.debian.org, and install it manually.
...
Figyelem There may be links to documentation elsewhere on the web. Try not to change the urls of the documentation pages. Or at the very least, put links to the new location on the previous url.
...
Infó This documentation use an anonymous account to connect the bridge to the chat. But since the livechat v10.1.0, there is a new way to generate long term authentication token, that allows to connect using your account. This is used for OBS docks. Using this feature for other purposes is not documented and not officially supported yet. If you want to use it anyway, you can request a token by calling then /plugins/livechat/router/api/auth/tokens endpoint. To get needed headers and request body, just check what happens when you generate a new token for OBS docks.`,description:"Using Matterbridge to bridge with other chats",tags:[],title:"Using Matterbridge",uri:"/peertube-plugin-livechat/hu/documentation/admin/advanced/matterbridge/index.html"},{breadcrumb:"Peertube livechat",content:`Interested in contributing? Awesome!
Code of ConductContributor Covenant Code of Conduct
TranslateTranslate the plugin
Give your feedbackGive your feedback
DevelopDevelop
DocumentationDocument the plugin, or translate the documentation.`,description:"Contributing",tags:[],title:"Contributing",uri:"/peertube-plugin-livechat/hu/contributing/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The livechat plugin includes a “slow mode” feature, to rate limit the number of messages that a user can send to a given MUC room. At time of writing, there were no XEP to describe such feature. Please find below a XEP draft, that will be submitted for review.
Figyelem Work In Progress, this page is not done yet. For an updated version of this document, you can check the draft XEP XMP file.
...
Appendix F: Requirements Conformance The following requirements keywords as used in this document are to be interpreted as described in RFC 2119: “MUST”, “SHALL”, “REQUIRED”; “MUST NOT”, “SHALL NOT”; “SHOULD”, “RECOMMENDED”; “SHOULD NOT”, “NOT RECOMMENDED”; “MAY”, “OPTIONAL”.`,description:"MUC Slow mode XEP",tags:[],title:"MUC Slow mode",uri:"/peertube-plugin-livechat/hu/technical/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:`This chat plugin relies on the XMPP protocol (also known as Jabber). It is therefore possible to connect to the chats using XMPP client software. This can be useful for example to facilitate moderation operations.
Infó The features described on this page must be enabled and configured by your Peertube instance’s administrators. You may therefore not have access to them.
Login to your Peertube account Figyelem This feature is not yet available, and will come in a future version of the plugin.
...
Infó By default, the share button is only visible to the owner of the video, and the admins/moderators of the instance. However, admins can decide to display this button for everyone.
Then, choose “Connect using XMPP”:
Then you just have to click on “open” or copy/paste the address of the chat room into your XMPP client (using the “join a room” feature).`,description:"Connect to chat using a XMPP client",tags:[],title:"XMPP Clients",uri:"/peertube-plugin-livechat/hu/documentation/user/xmpp_clients/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The poll system relies on two thinks:
...
And thanks to all individual contributors who have made a donation via my liberapay page.`,description:"Plugin Credits",tags:[],title:"Credits",uri:"/peertube-plugin-livechat/hu/credits/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:`Enabling the chat for you live streams Figyelem Instance administrators can choose to disable or enable chat in specific cases. Information in this section are only true in the default case.
...
Tipp There can be other settings in this tab, depending on plugins installed on your Peertube instance.
...
Forbidden wordsThe bot can automatically moderate messages containing forbidden words.
TimersThe bot can send periodically some messages.
CommandsThe bot can respond to several commands.`,description:"How to setup the chat for your live stream",tags:[],title:"For streamers",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Figyelem This section is still incomplete.
Figyelem This page describes the behaviour of livechat versions >= 10.0.0. There were some changes in the way we manage access rights for Peertube administrators and moderators.
...
Tipp The video owner will be owner of the chat room. This means they can configure the room, delete it, promote other users as admins, …
Tipp Starting with livechat v10.0.0, Peertube instance’s admins and moderators have no special rights on rooms by default. However, they have a special button available on top of the chat: “Become moderator”. Clicking this button will give them owner access on the room.
...
Mute anonymous users Infó This feature comes with the livechat plugin version 10.2.0.
...
Figyelem This section is still incomplete.
You can promote users as moderators, if you need some help.
Anonymize moderation actions Infó This feature comes with the livechat plugin version 11.0.0.
...
You can choose to enable or disable this feature for new chatrooms on the channel configuration page.
Participant message history search Infó This feature comes with the livechat plugin version 11.0.0.
As a room admin or owner, you can search all messages sent by a given participant.
To do so, you have several ways:
using the “Search all messages” action in the dropdown menu besides participants in the sidebar using the “Search all messages” action in the dropdown menu besides chat messages Tipp To have more space and better readability, open the chat in full-page mode.
...
From there, you can also promote yourself as room moderator by using the “Become moderator” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Infó This feature comes with the livechat plugin version 10.2.0.
...
Infó Peertube instance’s admin can also set global terms & conditions. If so, these terms will be shown above your channel’s terms.
Infó Anonymous users will only see the terms & conditions once they have chosen their nickname (in other words: once they are able to talk).
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Infó This feature comes with the livechat plugin version 10.3.0.
...
Figyelem Currently, this feature has one known bug: users that join the chat will get all messages, even messages that are still pending for other participants. However, messages sent after they joined will be delayed correctly.
Tipp You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Infó This feature comes with the livechat plugin version 10.1.0.
...
Tipp Anonymous viewers can only vote once they have choosen their nickname.
...
Infó For viewers using XMPP clients or outdated livechat plugin versions, the banner will not be visible. But they will see the message in the chat and will be able to vote by sending messages with their choices.
When the poll ends, a new message will be sent in the chat, with the results.
Infó The only way to get old polls results is to search for the poll end message in the chat. For now, polls results are not saved by any other means. So don’t forget to note polls results if you want to keep them.`,description:"You can create polls to ask viewers their opinion.",tags:[],title:"Polls",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/polls/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Infó This feature comes with the livechat plugin version 10.0.0.
...
Tipp To have more space and better readability, open the chat in full-page mode.
...
Tipp All modification are instantly visible in all your browser tabs, and for all room’s admins.
...
Figyelem Currently the video vs channel rooms is an instance-wide settings. Only Peertube admins can change it, and it applies to all chatrooms. In the future, this choice will be added in your channel’s options.
...
Sorting notes You can sort notes simply using drag & drop.`,description:"Plugin peertube-plugin-livechat moderation notes",tags:[],title:"Moderation notes",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/moderation_notes/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Infó This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
The bot will reload instantly when you save the page.`,description:"Chat bot setup",tags:[],title:"Chat bot",uri:"/peertube-plugin-livechat/hu/documentation/user/streamers/bot/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Kategóriák",uri:"/peertube-plugin-livechat/hu/categories/index.html"},{breadcrumb:"",content:`PeerTube plugin livechat Tipp You can use the language selector in the left menu to view this documentation in different languages. Some translations are missing or incomplete. In this case, you’ll see the English version of the text.
...
Tipp You can use the searchbox in the left menu to quickly find specific documentation parts.
Infó Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
...
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/hu/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Címkék",uri:"/peertube-plugin-livechat/hu/tags/index.html"}]