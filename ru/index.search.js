...
Совет You can test the livechat plugin with this demo page.
...
You can even activate the chat for specific VOD videos. This is how the demo page works: it is not a live stream, but I have activated the chat specifically for this video.`,description:"Introduction",tags:[],title:"Introduction",uri:"/peertube-plugin-livechat/ru/intro/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:` Совет This Code of Conduct is adapted from the Contributor Covenant, version 2.1, available at https://www.contributor-covenant.org/version/2/1/code_of_conduct.html. Translations are available at https://www.contributor-covenant.org/translations. Instances of abusive, harassing, or otherwise unacceptable behavior may be reported to the community leaders responsible for enforcement by mail at git.[at].john-livingston.fr.
...
If you haven’t a Peertube account Внимание This feature can be disabled by the instance’s adminitrators.
...
Log in using an external authentication provider Внимание This feature can be disabled by the instance’s adminitrators.
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/ru/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation",content:`This section describes the plugin settings page.
...
Информация Changing this setting will restart the chat server, and all users will be disconnected for a short time.
Показать существующие комнаты When pressing the «List rooms» button, all existing chatrooms will be listed. You can then find them and moderated them.
Federation Following settings concern the federation with other Peertube instances, and other fediverse softwares.
Не показывать внешние чаты Имея эту настройку, ваш сервер никогда не будет отображать чаты на видео с внешних серверов.
Не публиковать информацию чата By checking this setting, your instance will not publish chat information on the fediverse. Remote Peertube instances will not be aware that they are chat rooms associated to your videos.
...
Chat behaviour Тип комнаты Здесь вы можете выбрать, чтобы каждое видео имело отдельную комнату или чтобы чат-комната распротранялась на канал.
Автоматически открывать чат При просмотре видео чат будет открываться автоматически.
Показать кнопку «открыть в новом окне» Появится кнопка для открытия чата в новом окне.
...
Спрятать чат от анонимусов If checked, anonymous Peertube users won’t see the chat. This feature is still experimental. If you enabled it, it is highly recommended to also check “Don’t publish chat information”. Otherwise, some third party tools could try to open the chat, and have unpredictable behaviours.
...
Peertube theme: this is a special theme, made especially for peertube’s integration. Default ConverseJS theme: this is the default ConverseJS theme. ConverseJS cyberpunk theme: this is a theme provided by ConverseJS. Автоопределение цвета Попытаться автоматически определить цвета из текущей темы пользователя.
Когда этот параметр включен, плагин пытается автоматически определить цвета, которые будут применены к теме чата.
Если это не работает правильно для некоторых ваших тем Peertube, вы можете отключить эту опцию. Вы можете сообщить об ошибке на официальном трекере проблем . Не забудьте указать, какая тема не работает.
Аттрибут стиля IFrame для Веб-чата Additional styles to be added on the iframe style attribute. Example: height:400px;
Chat server advanced settings Использовать системный Prosody The plugin comes with an AppImage that is used to run the Prosody XMPP server. If this AppImage is not working, you can fallback to the Prosody that is packaged for your server. Just install the prosody package.
This setting should only be used if the plugin is broken, and waiting for a patch.
Отключить вебсокеты With Peertube >= 5.0.0, this plugin try to use Websocket connection for chatting. If the user’s browser or connection is incompatible, the browser will automatically fall back on the BOSH protocol. But in rare case, this can fail. For example, if you have a reverse proxy in front of Peertube that does not allow Websocket connection for plugins. In this case, you can check this setting to disable Websocket connections.
...
Examples:
*, :: * 127.0.0.1, ::1 172.18.0.42 Папка с сертификатами If this field is empty, the plugin will generate and use self-signed certificates.
...
If you can’t fix this immediatly, you can disable Websocket by unchecking “Отключить вебсокеты” in the plugin setting page. In such case, you should also check “Не публиковать информацию чата”, as chat federation won’t work without Websocket.`,description:"Some classic mistakes and workarounds.",tags:[],title:"Troubleshooting",uri:"/peertube-plugin-livechat/ru/documentation/installation/troubleshooting/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` For viewersHow to chat for stream viewers
OBSDocumentation to stream the chat content using OBS.
XMPP ClientsConnect to chat using a XMPP client
For streamersHow to setup the chat for your live stream
Some basicsSome basics about how to setup and use the chat for your live stream
...
OpenID Connect Внимание This feature is still experimental. This feature is available with the plugin version >= 9.0.0.
...
More to come Other authentication methods will be implemented in the future.`,description:"Plugin Peertube Livechat settings - External Authentication",tags:[],title:"External Authentication",uri:"/peertube-plugin-livechat/ru/documentation/admin/external_auth/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`You can configure some words that will be automatically moderated by the bot (messages containing such words will be instantly deleted). You can also add an optional reason that will be displayed at the place of deleted messages. Several examples are provided on the documentation page.
...
Совет These words are case insensitive.
Совет You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
Внимание This features is still experimental. There might be some issues with non-latin alphabets. You can open an issue to report your problems.
Consider as regular expressions Внимание This feature can be disabled by the instance’s adminitrators.
...
Reason Reason to display besides deleted messages
Комментарии You can add here some comments about this rule, to remember how and why you created it. These comments are purely indicative, and have no influence on the bot’s behavior.`,description:"The bot can automatically moderate messages containing forbidden words.",tags:[],title:"Forbidden words",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/bot/forbidden_words/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` Информация Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
Совет To install or update the plugin, just use the Peertube web admin interface.
Here are some other more specific instructions:
TroubleshootingSome classic mistakes and workarounds.
Known issues: CPU CompatibilityFor now, the plugin only works out of the box for x86_64 and arm64 CPU architecture. Here are some instructions for other CPU architectures.
Upgrade from version older than 6.0.0Important notes when upgrading for an older version.`,description:"Plugin peertube-plugin-livechat installation guide",tags:[],title:"Installation guide",uri:"/peertube-plugin-livechat/ru/documentation/installation/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`The Prosody AppImage included in the plugin will only work on x86_64 and arm64 CPU. It is not compatible with other CPU architectures.
...
opening a web page (with an url like https://yourinstance.tld/plugins/livechat/router/webchat/room/8df24108-6e70-4fc8-b1cc-f2db7fcdd535), using a XMPP client (and joining a room like xmpp://8df24108-6e70-4fc8-b1cc-f2db7fcdd535@room.yourinstance.tld?join) Внимание Joining the chat using a XMPP client is not always possible. It requires some DNS and server configuration. It will only be possible if instance’s admins have correctly setup the external XMPP clients connection feature.
Внимание Don’t try to gues these url and connection methods yourself. Please report to next chapters.
...
Информация This requires Peertube >= 5.1
This follows the FEP-1970 recommendations.
Внимание At the time of the writing, this FEP is in draft status, and the livechat plugin is a Proof-of-concept. Until the FEP is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Информация This requires Peertube >= 5.2
Информация The <podcast:chat> element is currently only supported for live streams.
This follows the <podcast:chat> proposal.
Внимание At the time of the writing, this proposal is in draft status, and the livechat plugin is a Proof-of-concept. Until the proposal is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Send every X minutes The bot will post the message every X minutes.`,description:"The bot can send periodically some messages.",tags:[],title:"Timers",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/bot/quotes/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation > Advanced usage",content:`This chat module is based on the XMPP protocol, also known as Jabber. It is therefore possible to connect to the chats using XMPP client software. This can for example be useful to facilitate moderation operations.
For the user documentation associated with these features, please refer to the user documentation page.
Информация Enabling these features requires configuration changes on the server, and on the DNS records. It is not possible to configure this only from the Peertube interface, and it requires some basic system some basic system admin skills.
Login to your Peertube account Внимание This feature is not yet available, and will come in a future version of the plugin.
...
Внимание Don’t enable this mode on a production server, neither on a public server. This could cause security issues.
...
OBS Dock Информация This feature comes with the livechat plugin version 10.1.0. Внимание This feature can be disabled by the instance’s adminitrators.
...
To do so, just use the “Поделиться ссылкой на чат” feature, and open the “Dock” tab. From there, you can create a new token using the “+” button.
...
Совет Tokens are valid to join any chat room. You don’t have to generate separate tokens for each of your rooms. You can also customize the nickame that will be used by changing the n parameter in the url.
...
Внимание Some linux distro (like Debian Bullseye for example) have too old version of po4a. Please make sure to install a compatible version. If you are using Debian Bullseye for example, you can download the Bookworm po4a.deb file from https://packages.debian.org, and install it manually.
...
Внимание There may be links to this documentation elsewhere on the web. Try not to change the urls of the documentation pages. Or at the very least, put links to the new location on the previous url.
...
Информация This feature comes with the livechat plugin version 12.0.0. What if I can’t use hugo and/or po4a? Just edit english markdown files, and specify that you can’t build translations when you make your Pull Request.
Publication Publishing the documentation is automatic, as soon as the changes are merged into the documentation branch.`,description:"Document the plugin, or translate the documentation.",tags:[],title:"Documentation",uri:"/peertube-plugin-livechat/ru/contributing/document/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The livechat plugin stores some data on the server, in the /var/www/peertube/storage/plugins/data/peertube-plugin-livechat/ folder. This page describes these data.
...
Информация This documentation use an anonymous account to connect the bridge to the chat. But since the livechat v10.1.0, there is a new way to generate long term authentication token, that allows to connect using your account. This is used for OBS docks. Using this feature for other purposes is not documented and not officially supported yet. If you want to use it anyway, you can request a token by calling then /plugins/livechat/router/api/auth/tokens endpoint. To get needed headers and request body, just check what happens when you generate a new token for OBS docks.`,description:"Using Matterbridge to bridge with other chats",tags:[],title:"Using Matterbridge",uri:"/peertube-plugin-livechat/ru/documentation/admin/advanced/matterbridge/index.html"},{breadcrumb:"Peertube livechat",content:`Interested in contributing? Awesome!
Code of ConductContributor Covenant Code of Conduct
TranslateTranslate the plugin
Give your feedbackGive your feedback
DevelopDevelop
DocumentationDocument the plugin, or translate the documentation.`,description:"Contributing",tags:[],title:"Contributing",uri:"/peertube-plugin-livechat/ru/contributing/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The livechat plugin includes a “slow mode” feature, to rate limit the number of messages that a user can send to a given MUC room. At time of writing, there were no XEP to describe such feature. Please find below a XEP draft, that will be submitted for review.
Внимание Work In Progress, this page is not done yet. For an updated version of this document, you can check the draft XEP XMP file.
...
Appendix F: Requirements Conformance The following requirements keywords as used in this document are to be interpreted as described in RFC 2119: “MUST”, “SHALL”, “REQUIRED”; “MUST NOT”, “SHALL NOT”; “SHOULD”, “RECOMMENDED”; “SHOULD NOT”, “NOT RECOMMENDED”; “MAY”, “OPTIONAL”.`,description:"MUC Slow mode XEP",tags:[],title:"MUC Slow mode",uri:"/peertube-plugin-livechat/ru/technical/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:`This chat plugin relies on the XMPP protocol (also known as Jabber). It is therefore possible to connect to the chats using XMPP client software. This can be useful for example to facilitate moderation operations.
Информация The features described on this page must be enabled and configured by your Peertube instance’s administrators. You may therefore not have access to them.
Login to your Peertube account Внимание This feature is not yet available, and will come in a future version of the plugin.
...
Информация By default, the share button is only visible to the owner of the video, and the admins/moderators of the instance. However, admins can decide to display this button for everyone.
Then, choose “Подключиться по XMPP”:
Then you just have to click on “open” or copy/paste the address of the chat room into your XMPP client (using the “join a room” feature).`,description:"Connect to chat using a XMPP client",tags:[],title:"XMPP Clients",uri:"/peertube-plugin-livechat/ru/documentation/user/xmpp_clients/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The poll system relies on two thinks:
...
And thanks to all individual contributors who have made a donation via my liberapay page.`,description:"Plugin Credits",tags:[],title:"Credits",uri:"/peertube-plugin-livechat/ru/credits/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:`Enabling the chat for you live streams Внимание Instance administrators can choose to disable or enable chat in specific cases. Information in this section are only true in the default case.
When you create or modify a Peertube live, there is a “plugin settings” tab:
In the “plugin settings” tab, there is a “Использовать чат” checkbox. Just check or uncheck it to enable or disable the chat associated to your video.
Совет There can be other settings in this tab, depending on plugins installed on your Peertube instance.
...
Share the chat On top of the chat, there is a “Поделиться ссылкой на чат” button.
This button opens a popup, where you can obtain an url to join the chat. This url can be shared.
The “Встроить” tab provide some links to embed the chat in websites, or in your live stream.
You can customize some options:
Только для чтения: you will only be able to read the chat, not write. This is useful to include the chat content in your live stream (see the OBS documentation). Использовать цвета из текущей темы: if checked, your current theme colors will be added to the url, so that any user that opens the link will have the same color set. Создать iframe для интеграци чата в веб-сайт: instead of an url, you will obtain an HTML snippet that you can add to your website to embed the chat. For more information on the “Dock” tab, check the OBS documentation.
In the “Веб” tab, the provided url opens the chat in the Peertube interface. You can share this link to other users to invite them to join the chat.
The “Поделиться ссылкой на чат” popup can also contain a “Подключиться по XMPP” tab. This will only be available if your instance’s administators have enabled an correctly configured this option. Using this option, you can provide a link to join the chat using any XMPP client software. Using such softwares can for example facilitate moderation actions.
...
The chat will be automatically recreated each time someone tries to join it as long as the video exists, and has the “Использовать чат” feature activated.`,description:"Some basics about how to setup and use the chat for your live stream",tags:[],title:"Some basics",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/basics/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Информация This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
In the Peertube left menu, there is a “Чат-комнаты” entry:
This “Чат-комнаты” link takes you to a list of your channels. By clicking on a channel, you will then be able to setup some options for your channels:
...
Совет The video owner will be owner of the chat room. This means they can configure the room, delete it, promote other users as admins, …
Совет Starting with livechat v10.0.0, Peertube instance’s admins and moderators have no special rights on rooms by default. However, they have a special button available on top of the chat: “Become moderator”. Clicking this button will give them owner access on the room.
...
Mute anonymous users Информация This feature comes with the livechat plugin version 10.2.0. You can prevent anonymous users to send messages. In such case, only registered users will be able to talk in the chat.
...
Внимание This section is still incomplete.
You can promote users as moderators, if you need some help.
Anonymize moderation actions Информация This feature comes with the livechat plugin version 11.0.0. It is possible to anonymize moderation actions, to avoid disclosing who is banning/kicking/… occupants.
...
You can choose to enable or disable this feature for new chatrooms on the channel configuration page.
Participant message history search Информация This feature comes with the livechat plugin version 11.0.0. As a room admin or owner, you can search all messages sent by a given participant.
To do so, you have several ways:
using the “Search all messages” action in the dropdown menu besides participants in the sidebar using the “Search all messages” action in the dropdown menu besides chat messages Совет To have more space and better readability, open the chat in full-page mode.
...
Информация Peertube instance’s admin can also set global terms & conditions. If so, these terms will be shown above your channel’s terms.
Информация Anonymous users will only see the terms & conditions once they have chosen their nickname (in other words: once they are able to talk).
...
Внимание Currently, this feature has one known bug: users that join the chat will get all messages, even messages that are still pending for other participants. However, messages sent after they joined will be delayed correctly.
Совет You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Информация This feature comes with the livechat plugin version 10.1.0. Channel emojis Streamers can add custom emojis to their channels.
...
[ { "sn": ":short_name:", "url": "https://example.com/image.png" } ] The sn attribute is the short name code. The url attribute can be any image url than your browser can access, or a Data URL representing the file you want to import.`,description:"Plugin peertube-plugin-livechat custom emojis",tags:[],title:"Custom emojis",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/emojis/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Информация This feature comes with the livechat plugin version 12.0.0. Emojis only mode You can enable an “Emojy only mode” in your chatrooms. When this mode is enabled, participants can only send emojis (standard, or channel custom emojis). Moderators are not affected by this limitation.
...
Совет Anonymous viewers can only vote once they have choosen their nickname.
...
Информация For viewers using XMPP clients or outdated livechat plugin versions, the banner will not be visible. But they will see the message in the chat and will be able to vote by sending messages with their choices.
When the poll ends, a new message will be sent in the chat, with the results.
Информация The only way to get old polls results is to search for the poll end message in the chat. For now, polls results are not saved by any other means. So don’t forget to note polls results if you want to keep them.`,description:"You can create polls to ask viewers their opinion",tags:[],title:"Polls",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/polls/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Информация This feature comes with the livechat plugin version 10.0.0. Introduction The livechat plugin includes a Task Application: a kind of “to-do list” feature where you can create task lists and add tasks to them. Every room’s admins have access to these tasks, so you can edit them collaboratively.
...
Совет To have more space and better readability, open the chat in full-page mode.
...
Совет All modification are instantly visible in all your browser tabs, and for all room’s admins.
...
Внимание Currently the video vs channel rooms is an instance-wide settings. Only Peertube admins can change it, and it applies to all chatrooms. In the future, this choice will be added in your channel’s options.
...
Sorting notes You can sort notes simply using drag & drop.`,description:"Plugin peertube-plugin-livechat moderation notes",tags:[],title:"Moderation notes",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/moderation_notes/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Информация This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
The bot will reload instantly when you save the page.`,description:"Chat bot setup",tags:[],title:"Chat bot",uri:"/peertube-plugin-livechat/ru/documentation/user/streamers/bot/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Категории",uri:"/peertube-plugin-livechat/ru/categories/index.html"},{breadcrumb:"",content:` Совет You can use the language selector in the left menu to view this documentation in different languages. Some translations are missing or incomplete. In this case, you’ll see the English version of the text.
...
Совет You can use the searchbox in the left menu to quickly find specific documentation parts.
Информация Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
...
Связаться со мнойСвязаться с автором
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/ru/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Теги",uri:"/peertube-plugin-livechat/ru/tags/index.html"}]