...
ヒント You can test the livechat plugin with this demo page.
...
If you haven’t a Peertube account 警告 This feature can be disabled by the instance’s adminitrators.
...
Log in using an external authentication provider 警告 This feature can be disabled by the instance’s adminitrators.
The Peertube instance can configure external authentication providers (Mastodon accounts, Google accounts, …). In such case, you will see a “外部アカウントを使用してログイン” button, that will open a dialog modal. In this dialog modal, there will be some buttons to connect using a remote account.
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/ja/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation",content:`This section describes the plugin settings page.
チャンネルのチャットルームの利用規約 ユーザーがチャットルームに参加する際に表示される"利用規約"メッセージを設定します。
For more information on this feature, check the documentation for channel’s terms & conditions.
情報 Changing this setting will restart the chat server, and all users will be disconnected for a short time.
...
«チャットリンクを共有»ボタンを表示する This feature enables a «share chat link» modal. With this modal, you can generate URLs to join the chat. The chat can be customized (readonly mode, use the current theme, …).
...
ユーザー自身でライブ配信のチャットを有効できるようにする チェックを入れると、すべてのライブ配信に対して、チャットを有効にするためのチェックボックスが表示されるようになります。
The video owner will be able to activate web chats.
すべてのライブ配信でチャットを有効にする チェックを入れると、すべてのライブ配信でチャットが有効になります。
全ての非ライブ配信でチャットを有効にする 全ての非ライブ配信でチャットを有効にする
次の動画でチャットを有効にする 次の動画でチャットを有効にする
...
ユーザーがチャットルームから追放された場合に匿名ユーザーのIPを凍結する このオプションを有効にすると、匿名のユーザーがチャットルームから追放されるたびに、そのユーザーのIPもチャットサーバーから追放されます。 警告: あなたのインスタンスが登録を受け付けている場合、どんなユーザーでも罠のルームを作成し、他のユーザーを招待し、自動的にすべての匿名ユーザーのIPを追放できます。 追放されたIPリストは保存されず、サーバー再起動時やプラグインの設定を変更した場合にクリアされます。 追放されたIPはProsodyサーバーログファイルに記録されるため、サーバーの管理者は外部ツール（fail2banなど）を使用してIPを広範囲に追放することができます。
...
Theming アバターセット You can choose from several different sets the default avatars that will be used for chat users.
...
vendor The vendor folder is not part of the source code. It is used during the build process to download some external source code.`,description:"Source code organization",tags:[],title:"Source code",uri:"/peertube-plugin-livechat/ja/technical/sourcecode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`特殊文字を禁止する 情報 This feature comes with the livechat plugin version 11.1.0. Configuration このオプションを有効にすると、モデレーションBotは、X文字以上の特殊文字が含まれるメッセージを自動的に削除します。 特殊文字は、文字、数字、句読点記号、通貨記号、絵文字のいずれにも当てはまらないものを指します。
許容範囲 削除しない特殊文字の文字数を指定してください。
理由 削除理由
モデレーターからのメッセージも対象にする 初期状態では、モデレーターメッセージはこの機能の影響を受けません。 このオプションを有効にすると、モデレーターからのメッセージも削除されるようになります。`,description:"The bot can automatically moderate messages containing too many special characters.",tags:[],title:"Special characters",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/bot/special_chars/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`I just installed/upgraded the plugin, but nothing happens If you have just installed/upgraded the plugin, but nothing happens (no chat, no settings, buttons in the settings page does not work, …), just try to reload the page.
...
In such case, try changing the “API呼び出しのためのPeerTube URL” settings, by setting http://127.0.0.1:9000 (assuming 9000 is the port on which Peertube listen, ask your instance administrators if you don’t know).
...
If you can’t fix this immediatly, you can disable Websocket by unchecking “Websocketを無効にする” in the plugin setting page. In such case, you should also check “チャット情報を公開しない”, as chat federation won’t work without Websocket.`,description:"Some classic mistakes and workarounds.",tags:[],title:"Troubleshooting",uri:"/peertube-plugin-livechat/ja/documentation/installation/troubleshooting/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` For viewersHow to chat for stream viewers
OBSDocumentation to stream the chat content using OBS.
XMPP ClientsConnect to chat using a XMPP client
For streamersHow to setup the chat for your live stream
Some basicsSome basics about how to setup and use the chat for your live stream
...
CommandsThe bot can respond to several commands.`,description:"Plugin peertube-plugin-livechat user documentation",tags:[],title:"User documentation",uri:"/peertube-plugin-livechat/ja/documentation/user/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:` ヒント この行動規範は、Contributor Covenantのバージョン 2.1を基に作成されており、https://www.contributor-covenant.org/version/2/1/code_of_conduct.htmlから入手できます。 翻訳はhttps://www.contributor-covenant.org/translationsにあります。 虐待的、嫌がらせ、またはその他の受け入れられない行為の事例は、執行を担当するコミュニティリーダーに、git.[at].john-livingston.fr宛のメールで報告することができます。
...
OpenID Connect 警告 This feature is still experimental. This feature is available with the plugin version >= 9.0.0.
...
To enable this feature, first you have to create a client on your provider side (check the related documentation for enabling OpenID Connect). Then go to the plugin settings, and enable “Open ID Connectプロバイダーを使用する”.
...
接続ボタンのラベル名 このラベルは、OIDCプロバイダーを使用して認証するボタン名としてユーザーに表示されます。
This is the button label in the following screenshot:
For now, it is not possible to localize this label.
ディスカバリーURL Your OpenID Connect provider must implement the discovery URL. Just set here the discovery url, that should be something like https://example.com/.well-known/openid-configuration.
...
クライアントID Your application Client ID.
クライアントシークレット You application Client secret.
...
ヒント These words are case insensitive.
...
ヒント To install or update the plugin, just use the Peertube web admin interface.
Here are some other more specific instructions:
TroubleshootingSome classic mistakes and workarounds.
Known issues: CPU CompatibilityFor now, the plugin only works out of the box for x86_64 and arm64 CPU architecture. Here are some instructions for other CPU architectures.
Upgrade from version older than 6.0.0Important notes when upgrading for an older version.`,description:"Plugin peertube-plugin-livechat installation guide",tags:[],title:"Installation guide",uri:"/peertube-plugin-livechat/ja/documentation/installation/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`The Prosody AppImage included in the plugin will only work on x86_64 and arm64 CPU. It is not compatible with other CPU architectures.
...
opening a web page (with an url like https://yourinstance.tld/plugins/livechat/router/webchat/room/8df24108-6e70-4fc8-b1cc-f2db7fcdd535), using a XMPP client (and joining a room like xmpp://8df24108-6e70-4fc8-b1cc-f2db7fcdd535@room.yourinstance.tld?join) 警告 Joining the chat using a XMPP client is not always possible. It requires some DNS and server configuration. It will only be possible if instance’s admins have correctly setup the external XMPP clients connection feature.
警告 Don’t try to gues these url and connection methods yourself. Please report to next chapters.
...
情報 This requires Peertube >= 5.1
This follows the FEP-1970 recommendations.
警告 At the time of the writing, this FEP is in draft status, and the livechat plugin is a Proof-of-concept. Until the FEP is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
情報 This requires Peertube >= 5.2
情報 The <podcast:chat> element is currently only supported for live streams.
This follows the <podcast:chat> proposal.
警告 At the time of the writing, this proposal is in draft status, and the livechat plugin is a Proof-of-concept. Until the proposal is adopted, the specification can change, and the livechat plugin will be adapted accordingly.
...
Examples Don’t hesitate to share your rules. To do so, you can for example edit this page.`,description:"Advanced firewall rules for the Prosody server",tags:[],title:"Prosody mod_firewall",uri:"/peertube-plugin-livechat/ja/documentation/admin/mod_firewall/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`一定間隔でメッセージを送信するための複数のタイマーを設定できます。 これらのメッセージは、BotによってX分ごとに送信されます。 たとえば、Botからスポンサー情報を5分ごとに送信させることができます。
ヒント If there is no user in the chatroom, the bot won’t send any message.
タイマー 一定間隔でメッセージを送信するための複数のタイマーを設定できます。 これらのメッセージは、BotによってX分ごとに送信されます。 たとえば、Botからスポンサー情報を5分ごとに送信させることができます。
メッセージ1つに対して1行で入力してください。 複数のメッセージが入力されている場合、X分おきにランダムに選択されます。
X分おきに送信する BotはメッセージをX分おきに送信します。`,description:"The bot can send periodically some messages.",tags:[],title:"Timers",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/bot/quotes/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation > Advanced usage",content:`This chat module is based on the XMPP protocol, also known as Jabber. It is therefore possible to connect to the chats using XMPP client software. This can for example be useful to facilitate moderation operations.
For the user documentation associated with these features, please refer to the user documentation page.
情報 Enabling these features requires configuration changes on the server, and on the DNS records. It is not possible to configure this only from the Peertube interface, and it requires some basic system some basic system admin skills.
Login to your Peertube account 警告 This feature is not yet available, and will come in a future version of the plugin.
...
OBS Dock 情報 This feature comes with the livechat plugin version 10.1.0. 警告 This feature can be disabled by the instance’s adminitrators.
...
To do so, just use the “チャットリンクを共有” feature, and open the “ドック” tab. From there, you can create a new token using the “+” button.
...
ヒント Tokens are valid to join any chat room. You don’t have to generate separate tokens for each of your rooms. You can also customize the nickame that will be used by changing the n parameter in the url.
...
情報 These tokens can be used for other purposes, as connecting to your account with XMPP bots or clients. This feature is not documented yet, and not officially supported. So use with care.
Mixing multiple chats in your live stream You can use the social_stream browser extension to mix multiple chat source (from Peertube, Twitch, Youtube, Facebook, …) and include their contents in your live stream. The compatibility with this plugin was added in recent versions.`,description:"Documentation to stream the chat content using OBS.",tags:[],title:"OBS",uri:"/peertube-plugin-livechat/ja/documentation/user/obs/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:`常に開発したい機能について話し合い、問題に取り組むためのイシューを作成または見つけてコメントし、作業を開始する前にコミュニティに通知し、そのイシューに取り組むことを宣言してください。
プルリクエストは、mainブランチで行ってください。
メモ 2023年3月までは、コントリビューションはdevelopブランチで行っていました。このプロシージャは現在非推奨になっています。
このプラグインをビルドするための前提要件 次の概念を理解しておくことを強く推奨いたします:
Git NodeJS NPM Typescript プラグインをビルドするには、次のパッケージが必要です:
git npm (>=8.x) nodejs (>=14.x) build-essential coreutils wget reuse このプラグインは、Prosody XMPPサーバー用のAppImageが必要です。 このAppImageは、Prosody AppImage sideprojectにより提供されています。 build-prosody.shスクリプトは、このリモートリポジトリに添付されているバイナリをダウンロードし、ダウンロードしたファイルのsha256ハッシュサムが正しいかどうかを確認します。
開発 リポジトリをクローン、プラグインをビルド、機能ブランチを作成:
# リポジトリをクローンしてください。 サブモジュールもクローンするために --recursive を付与するのを忘れないでください。 git clone https://github.com/JohnXLivingston/peertube-plugin-livechat.git --recursive cd peertube-plugin-livechat # NPM依存関係をインストールし、初回のモジュールのビルドを実施します: npm install # 変更後、プラグインをビルドします: npm run build # リポジトリからフォークした場合、リモートとして追加してください (例): git remote add me git@github.com:MY_GITHUB_ACCOUNT/peertube-plugin-livechat.git # 開発のために、ローカルブランチを作成してください (例): git checkout my_development # 注意: イシューに関連している場合は、ブランチ名に fix_1234 のような形式を付与します （1234はイシューの番号） # 変更を提案するため、ブランチをリポジトリにプッシュしてください (例): git push --set-upstream me my_development # その後、WebブラウザーからGitHubリポジトリに移動し、プルリクエストを送ってください(詳しくは以下をお読みください)Once you are ready to show your code to ask for feedback, submit a draft Pull Request. Once you are ready for a code review before merge, submit a Pull Request. In any case, please link your PR to the issues it solves by using the GitHub syntax: “fixes #issue_number”.
...
警告 Don’t enable this mode on a production server, neither on a public server. This could cause security issues.
...
ドキュメンテーションThen open your browser on the address http://localhost:1313/peertube-plugin-livechat/. This page will automatically refresh on each modification.
...
警告 Some linux distro (like Debian Bullseye for example) have too old version of po4a. Please make sure to install a compatible version. If you are using Debian Bullseye for example, you can download the Bookworm po4a.deb file from https://packages.debian.org, and install it manually.
...
警告 There may be links to documentation elsewhere on the web. Try not to change the urls of the documentation pages. Or at the very least, put links to the new location on the previous url.
...
Internal connections only (basic) You will need to enable クライアントからサーバー接続を有効にする in the livechat plugin settings.
...
情報 This documentation use an anonymous account to connect the bridge to the chat. But since the livechat v10.1.0, there is a new way to generate long term authentication token, that allows to connect using your account. This is used for OBS docks. Using this feature for other purposes is not documented and not officially supported yet. If you want to use it anyway, you can request a token by calling then /plugins/livechat/router/api/auth/tokens endpoint. To get needed headers and request body, just check what happens when you generate a new token for OBS docks.`,description:"Using Matterbridge to bridge with other chats",tags:[],title:"Using Matterbridge",uri:"/peertube-plugin-livechat/ja/documentation/admin/advanced/matterbridge/index.html"},{breadcrumb:"Peertube livechat",content:`Interested in contributing? Awesome!
コード規約コントリビューター規約の行動規範
TranslateTranslate the plugin
Give your feedbackGive your feedback
開発開発
DocumentationDocument the plugin, or translate the documentation.`,description:"Contributing",tags:[],title:"Contributing",uri:"/peertube-plugin-livechat/ja/contributing/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The livechat plugin includes a “slow mode” feature, to rate limit the number of messages that a user can send to a given MUC room. At time of writing, there were no XEP to describe such feature. Please find below a XEP draft, that will be submitted for review.
警告 Work In Progress, this page is not done yet. For an updated version of this document, you can check the draft XEP XMP file.
...
Appendix F: Requirements Conformance The following requirements keywords as used in this document are to be interpreted as described in RFC 2119: “MUST”, “SHALL”, “REQUIRED”; “MUST NOT”, “SHALL NOT”; “SHOULD”, “RECOMMENDED”; “SHOULD NOT”, “NOT RECOMMENDED”; “MAY”, “OPTIONAL”.`,description:"MUC Slow mode XEP",tags:[],title:"MUC Slow mode",uri:"/peertube-plugin-livechat/ja/technical/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:`This chat plugin relies on the XMPP protocol (also known as Jabber). It is therefore possible to connect to the chats using XMPP client software. This can be useful for example to facilitate moderation operations.
情報 The features described on this page must be enabled and configured by your Peertube instance’s administrators. You may therefore not have access to them.
Login to your Peertube account 警告 This feature is not yet available, and will come in a future version of the plugin.
...
情報 By default, the share button is only visible to the owner of the video, and the admins/moderators of the instance. However, admins can decide to display this button for everyone.
Then, choose “XMPPを使用して接続”:
Then you just have to click on “open” or copy/paste the address of the chat room into your XMPP client (using the “join a room” feature).`,description:"Connect to chat using a XMPP client",tags:[],title:"XMPP Clients",uri:"/peertube-plugin-livechat/ja/documentation/user/xmpp_clients/index.html"},{breadcrumb:"Peertube livechat > Technical documentation",content:`The poll system relies on two thinks:
...
And thanks to all individual contributors who have made a donation via my liberapay page.`,description:"Plugin Credits",tags:[],title:"Credits",uri:"/peertube-plugin-livechat/ja/credits/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:`Enabling the chat for you live streams 警告 Instance administrators can choose to disable or enable chat in specific cases. Information in this section are only true in the default case.
When you create or modify a Peertube live, there is a “plugin settings” tab:
In the “plugin settings” tab, there is a “チャットを使用する” checkbox. Just check or uncheck it to enable or disable the chat associated to your video.
ヒント There can be other settings in this tab, depending on plugins installed on your Peertube instance.
...
Share the chat On top of the chat, there is a “チャットリンクを共有” button.
This button opens a popup, where you can obtain an url to join the chat. This url can be shared.
The “埋め込み” tab provide some links to embed the chat in websites, or in your live stream.
You can customize some options:
読み取り専用にする: you will only be able to read the chat, not write. This is useful to include the chat content in your live stream (see the OBS documentation). 現在のテーマカラーを使用する: if checked, your current theme colors will be added to the url, so that any user that opens the link will have the same color set. Webサイトへのチャット埋め込みのためのiframeを生成: instead of an url, you will obtain an HTML snippet that you can add to your website to embed the chat. For more information on the “ドック” tab, check the OBS documentation.
...
The chat will be automatically recreated each time someone tries to join it as long as the video exists, and has the “チャットを使用する” feature activated.`,description:"Some basics about how to setup and use the chat for your live stream",tags:[],title:"Some basics",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/basics/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
In the Peertube left menu, there is a “チャットルーム” entry:
This “チャットルーム” link takes you to a list of your channels. By clicking on a channel, you will then be able to setup some options for your channels:
...
ヒント The video owner will be owner of the chat room. This means they can configure the room, delete it, promote other users as admins, …
ヒント Starting with livechat v10.0.0, Peertube instance’s admins and moderators have no special rights on rooms by default. However, they have a special button available on top of the chat: “モデレーターにする”. Clicking this button will give them owner access on the room.
...
匿名ユーザーをミュート 情報 This feature comes with the livechat plugin version 10.2.0. You can prevent anonymous users to send messages. In such case, only registered users will be able to talk in the chat.
To enable or disable this feature, use the chat dropdown menu, open the “configure” menu. In the form, you will find a “匿名ユーザーをミュート” checkbox.
Anonymous users won’t have the message field, and will see following prompt: “登録済みユーザーのみがメッセージを送信できます。”
...
モデレーション操作を匿名か 情報 This feature comes with the livechat plugin version 11.0.0. It is possible to anonymize moderation actions, to avoid disclosing who is banning/kicking/… occupants.
...
You can choose to enable or disable this feature for new chatrooms on the channel configuration page.
Participant message history search 情報 This feature comes with the livechat plugin version 11.0.0. As a room admin or owner, you can search all messages sent by a given participant.
To do so, you have several ways:
using the “全てのメッセージを検索” action in the dropdown menu besides participants in the sidebar using the “全てのメッセージを検索” action in the dropdown menu besides chat messages ヒント To have more space and better readability, open the chat in full-page mode.
...
From there, you can also promote yourself as room moderator by using the “モデレーターにする” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 10.2.0. Configuration You can add terms & conditions to your channel. These terms will be shown to all users joining the chat.
...
情報 Peertube instance’s admin can also set global terms & conditions. If so, these terms will be shown above your channel’s terms.
情報 Anonymous users will only see the terms & conditions once they have chosen their nickname (in other words: once they are able to talk).
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 10.3.0. Introduction As a streamer, you can choose to delay messages in the chat, to let some time to moderators to delete messages before they can even be read by other participants.
...
Moderation delay option On the channel configuration page, you can set the “モデレーションの遅延時間” option:
This value will apply as a default value for all your channel’s chatrooms.
Setting the value to 0 will disable the feature.
...
警告 Currently, this feature has one known bug: users that join the chat will get all messages, even messages that are still pending for other participants. However, messages sent after they joined will be delayed correctly.
ヒント You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 10.1.0. Channel emojis Streamers can add custom emojis to their channels.
...
Import / Export On the channel configuration page, there are an “インポート” and an “エクスポート” button. The “エクスポート” button generates a file than you can then import on another channel.
...
[ { "sn": ":short_name:", "url": "https://example.com/image.png" } ] The sn attribute is the short name code. The url attribute can be any image url than your browser can access, or a Data URL representing the file you want to import.`,description:"Plugin peertube-plugin-livechat custom emojis",tags:[],title:"Custom emojis",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/emojis/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 11.1.0. 絵文字のみモード チャットルームで"絵文字のみモード"を有効にできます。 このモードが有効な時は、参加者は絵文字（標準またはチャンネル専用のカスタム絵文字）のみを送信できます。 モデレーターはこの制限の影響を受けません。
This mode can be usefull for example:
To avoid spam or offensive message when you are not here to moderate. When there are too many speaking participants, and you can’t no more moderate correctly. To enable or disable this feature, use the chat dropdown menu, open the “configure” menu. In the form, you will find a “絵文字のみモード” checkbox.
If you want to enable it for all your chatrooms at once, open the channel emojis configuration page, and use the “絵文字のみモードを全てのチャンネルのチャットルームに適用する” button.`,description:"Plugin peertube-plugin-livechat emojis only mode",tags:[],title:"Emojis only mode",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/emojis_only/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 10.2.0. Create a poll You can create a new poll by using the “新規投票を作成” action in the chat top menu:
警告 This poll feature should not be considered as a reliable voting system. It is easy to cheat. There is no mechanism to prevent anonymous users to vote multiple times by just reloading the chat. Votes are never fully anonymous, someone having access to the server could see who voted for what choice.
Poll form Fill the form fields:
“質問”: the question to ask to you viewers “投票期間 (分)”: the duration for which viewers can vote “匿名の結果”: if checked, votes won’t be publicly visible in the chat “Choice N”: choices that will be presented to viewers You must at least fill the two first choices fields.
...
When you promote someone as room admin or owner, they gets instant access to the “新規投票を作成” action.
...
Every user that is not muted can vote. This means that you can prevent anonymous users to vote by using the “匿名ユーザーをミュート” feature.
...
ヒント Anonymous viewers can only vote once they have choosen their nickname.
If “匿名の結果” is checked, votes won’t be shown to other users. If unchecked, votes will be publicly visible as you will see message like “!1” in the chat.
情報 For viewers using XMPP clients or outdated livechat plugin versions, the banner will not be visible. But they will see the message in the chat and will be able to vote by sending messages with their choices.
When the poll ends, a new message will be sent in the chat, with the results.
情報 The only way to get old polls results is to search for the poll end message in the chat. For now, polls results are not saved by any other means. So don’t forget to note polls results if you want to keep them.`,description:"You can create polls to ask viewers their opinion",tags:[],title:"Polls",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/polls/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 情報 This feature comes with the livechat plugin version 10.0.0. Introduction The livechat plugin includes a Task Application: a kind of “to-do list” feature where you can create task lists and add tasks to them. Every room’s admins have access to these tasks, so you can edit them collaboratively.
You can for example use the Task Application to:
prepare a list of themes you want to discuss during your livestream, so you can be sure you won’t forget anything highlight questions from your viewers, so you can come back to them later without forgetting to answer them … Using the Task Application Opening the Task Application To open the Task Application, there is a “タスク” button in the top chat menu:
Clicking this button will toggle the Task Application display:
ヒント To have more space and better readability, open the chat in full-page mode.
...
ヒント All modification are instantly visible in all your browser tabs, and for all room’s admins.
...
警告 Currently the video vs channel rooms is an instance-wide settings. Only Peertube admins can change it, and it applies to all chatrooms. In the future, this choice will be added in your channel’s options.
...
using the “新規ノートを作成” action in the dropdown menu besides participants in the sidebar using the “新規ノートを作成” action in the dropdown menu besides chat messages When a note is associated to a participant, you will see their nickname and avatar on the top of the note.
Notes filtering You can filter notes to find all notes related to a given participant in several ways:
click on the “ノートを検索” button that is available on notes to find all notes related to the same participant click on the “ノートを検索” button in the dropdown menu besides participants in the sidebar click on the “ノートを検索” button in the dropdown menu besides chat messages You can remove the filter by clicking on the close button.
...
The bot will reload instantly when you save the page.`,description:"Chat bot setup",tags:[],title:"Chat bot",uri:"/peertube-plugin-livechat/ja/documentation/user/streamers/bot/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"カテゴリー",uri:"/peertube-plugin-livechat/ja/categories/index.html"},{breadcrumb:"",content:` ヒント You can use the language selector in the left menu to view this documentation in different languages. Some translations are missing or incomplete. In this case, you’ll see the English version of the text.
...
ヒント You can use the searchbox in the left menu to quickly find specific documentation parts.
情報 メジャーリリースのアップデート実施前に、リリースノートをお読み頂き、変更をご確認ください : 変更履歴.
...
お問い合わせ作者に問い合わせる
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/ja/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"タグ",uri:"/peertube-plugin-livechat/ja/tags/index.html"}]