...
提示 You can test the livechat plugin with this demo page.
...
You can even activate the chat for specific VOD videos. This is how the demo page works: it is not a live stream, but I have activated the chat specifically for this video.`,description:"Introduction",tags:[],title:"Introduction",uri:"/peertube-plugin-livechat/zh-hant/intro/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:` 提示 This Code of Conduct is adapted from the Contributor Covenant, version 2.1, available at https://www.contributor-covenant.org/version/2/1/code_of_conduct.html. Translations are available at https://www.contributor-covenant.org/translations. Instances of abusive, harassing, or otherwise unacceptable behavior may be reported to the community leaders responsible for enforcement by mail at git.[at].john-livingston.fr.
...
The Peertube instance can configure external authentication providers (Mastodon accounts, Google accounts, …). In such case, you will see a “使用外部帳戶登入” button, that will open a dialog modal. In this dialog modal, there will be some buttons to connect using a remote account.
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`您可以組態一些由機器人自動審核的單字（包含此類單字的訊息將立即刪除）。 您也可以新增一個可選原因，該原因將顯示在已刪除郵件的位置。 檔案頁面上提供了幾個範例。 You can fill several “禁止使用的單字或句子” fields. When a user sends a message that match the configured criteria, the message will automatically be deleted.
禁止使用的單字或句子 In this field, you can set several words, group of words, or “regular expressions”.
每行只能一個單字或句子。如果將多個單字放在一行上，它將只配取到包含整行的訊息。 Each time a user sends a message, these words will be tested. If the message containes one of them, the message will be deleted.
...
提示 These words are case insensitive.
...
Also moderate messages from moderators By default, moderator messages will not be deleted when containing forbidden words. By checking this option, messages from moderators will also be deleted. 原因 除已刪除訊息外還顯示的原因 Comments You can add here some comments about this rule, to remember how and why you created it. These comments are purely indicative, and have no influence on the bot's behavior. `,description:"The bot can automatically moderate messages containing forbidden words.",tags:[],title:"Forbidden words",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/bot/forbidden_words/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation",content:`This section describes the plugin settings page.
...
資訊 Changing this setting will restart the chat server, and all users will be disconnected for a short time.
列出現有聊天室 When pressing the «List rooms» button, all existing chatrooms will be listed. You can then find them and moderated them.
Federation Following settings concern the federation with other Peertube instances, and other fediverse softwares.
不顯示遠端聊天 透過檢查此設置，您的實例將永遠不會顯示來自遠端視訊的聊天。 不公開聊天資訊 透過檢查此設置，您的實例將不會在 fediverse 上發布聊天訊息。 遠端 Peertube 實例不會意識到它們是與您的視訊關聯的聊天室。 請注意：如果您已經進行過聊天，則該資訊可能已經發布。 您必須等待下一次視訊更新，訊息才會取消發布。 此外，如果您停用此設置，則必須等待視訊更新後才能顯示資訊。 再次發布。當現場活動恢復或停止時，會發生此更新。 請注意：此設定僅影響透過 ActivityPub 協定發布資訊。 它不會阻止遠端應用程式檢測聊天的存在並嘗試連接到它。 Authentication Disable livechat tokens In case you have any trouble with the long term authentication tokens, you can disable the feature here.
...
Chat behaviour 聊天室類型 您可以在此處選擇為每個影片設定單獨的聊天室，或按頻道對它們進行分組。 自動開啟聊天 觀看影片時，聊天視窗會自動開啟。 顯示“在新視窗中開啟”按鈕 此按鈕可在新視窗中開啟網路聊天。 顯示「共享聊天連結」按鈕 This feature enables a «share chat link» modal. With this modal, you can generate URLs to join the chat. The chat can be customized (readonly mode, use the current theme, …).
...
使用者在直播時可啟用聊天 If checked, all live videos will have a checkbox in their properties for enabling the web chat. The video owner will be able to activate web chats.
所有直播啟動聊天室 若勾選，則所有直播都啟用聊天室。 Activate chat for all non-lives Activate chat for all non-lives Activate chat for these videos Activate chat for these videos 隱藏匿名使用者的聊天 若勾選，匿名 Peertube 用戶將看不到聊天資訊。 此功能仍處於實驗階段。 如果您啟用了它，強烈建議您也勾選「不發布聊天訊息」。 否則，某些第三方工具可能會嘗試開啟聊天，並出現不可預測的行為。 Note: for now this feature simply hide the chat. In a future release, the chat will be replaced by a message saying «please log in to […]». See v5.7.0 Release Notes for more information.
當用戶被禁止進入聊天室時，禁止匿名用戶的 IP 透過啟用此選項，每次匿名用戶被禁止進入聊天室時，其 IP 也將被聊天伺服器禁止。 警告：如果您的執行個體開放註冊，任何使用者都可以建立陷阱房間、邀請使用者加入，並自動禁止所有匿名使用者的 IP。 禁止的IP清單不會被存儲，它會在伺服器重新啟動或更改某些外掛的設定時被清除。 被禁止的IP會記錄在Prosody伺服器日誌檔案中，因此伺服器管理員最終可以使用一些外部工具（如fail2ban）來更廣泛地禁止IP。 Important note: If you enable this feature, and are using a custom reverse proxy on front of Peertube, please make sure that your setup is correctly configured to forward real user’s IPs to Peertube. Otherwise it could block all anonymous users at once.
...
Peertube theme: this is a special theme, made especially for peertube’s integration. Default ConverseJS theme: this is the default ConverseJS theme. ConverseJS concord theme: this is a theme provided by ConverseJS. 自動色彩檢測 嘗試自動偵測使用者目前主題的顏色。 啟用此設定後，外掛程式會嘗試自動偵測顏色以套用至聊天主題。 如果這對於您的某些 Peertube 主題無法正常運作，您可以停用此選項。 您可以向官方報告該錯誤 問題追蹤器 。不要忘記指定哪個主題不起作用。 聊天 iframe 風格屬性 Additional styles to be added on the iframe style attribute. Example: height:400px; Chat server advanced settings Use system Prosody The plugin comes with an AppImage that is used to run the Prosody XMPP server. If this AppImage is not working, you can fallback to the Prosody that is packaged for your server. Just install the prosody package.
This setting should only be used if the plugin is broken, and waiting for a patch.
停用 Websocket 當 Peertube >= 5.0.0 時，外掛嘗試使用 Websocket 連線進行聊天。 如果使用者的瀏覽器或連線不相容，瀏覽器將自動轉而使用 BOSH 協定。 但在極少數情況下，這可能會失敗。例如，如果您在 Peertube 前面有一個反向代理，但它不 允許外掛的 Websocket 連線。 在這種情況下，您可以檢查此設定以停用 Websocket 連線。 Prosody port The port that will be used by the Prosody server.
Change it if this port is already in use on your server.
You can close this port on your firewall, it will not be accessed from the outer world.
Note: this might change in a near future, as it is planned to add a feature to activate external connections. Peertube API呼叫網址 Please let this setting empty if you don't know what you are doing.
...
vendor The vendor folder is not part of the source code. It is used during the build process to download some external source code.`,description:"Source code organization",tags:[],title:"Source code",uri:"/peertube-plugin-livechat/zh-hant/technical/sourcecode/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`I just installed/upgraded the plugin, but nothing happens If you have just installed/upgraded the plugin, but nothing happens (no chat, no settings, buttons in the settings page does not work, …), just try to reload the page.
...
In such case, try changing the “Peertube API呼叫網址” settings, by setting http://127.0.0.1:9000 (assuming 9000 is the port on which Peertube listen, ask your instance administrators if you don’t know).
...
連接按鈕的標籤 此標籤將作為向該 OIDC 提供者進行身份驗證的按鈕標籤顯示給使用者。 This is the button label in the following screenshot:
For now, it is not possible to localize this label.
OIDC_custom_dicovery_url Your OpenID Connect provider must implement the discovery URL. Just set here the discovery url, that should be something like https://example.com/.well-known/openid-configuration.
...
資訊 This requires Peertube >= 5.2
資訊 The <podcast:chat> element is currently only supported for live streams.
...
資訊 Enabling these features requires configuration changes on the server, and on the DNS records. It is not possible to configure this only from the Peertube interface, and it requires some basic system some basic system admin skills.
...
Using MatterbridgeUsing Matterbridge to bridge with other chats`,description:"Some advanced features",tags:[],title:"Advanced usage",uri:"/peertube-plugin-livechat/zh-hant/documentation/admin/advanced/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`You can configure the bot to respond to commands. A command is a message starting with a "!", like for example "!help" that calls the "help" command. You can setup several commands.
命令 The command, without the starting "!". For example "help", "sponsor", ... Send every X minutes The bot will post the message every X minutes. `,description:"The bot can respond to several commands.",tags:[],title:"Commands",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/bot/commands/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:`Always talk about features you want to develop by creating/finding and commenting the issue tackling your problem before you start working on it, and inform the community that you begin coding by claiming the issue.
Pull Request must be done on the main branch.
註釋 Until march 2023, contribution were made on the develop branch. This procedure is now deprecated.
...
OBS Dock 資訊 This feature comes with the livechat plugin version 10.1.0.
...
To do so, just use the “分享聊天連結” feature, and open the “Dock” tab. From there, you can create a new token using the “+” button.
...
提示 Tokens are valid to join any chat room. You don’t have to generate separate tokens for each of your rooms. You can also customize the nickame that will be used by changing the n parameter in the url.
...
Appendix F: Requirements Conformance The following requirements keywords as used in this document are to be interpreted as described in RFC 2119: “MUST”, “SHALL”, “REQUIRED”; “MUST NOT”, “SHALL NOT”; “SHOULD”, “RECOMMENDED”; “SHOULD NOT”, “NOT RECOMMENDED”; “MAY”, “OPTIONAL”.`,description:"MUC Slow mode XEP",tags:[],title:"MUC Slow mode",uri:"/peertube-plugin-livechat/zh-hant/technical/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:`This chat plugin relies on the XMPP protocol (also known as Jabber). It is therefore possible to connect to the chats using XMPP client software. This can be useful for example to facilitate moderation operations.
資訊 The features described on this page must be enabled and configured by your Peertube instance’s administrators. You may therefore not have access to them.
Login to your Peertube account 警告 This feature is not yet available, and will come in a future version of the plugin.
...
資訊 By default, the share button is only visible to the owner of the video, and the admins/moderators of the instance. However, admins can decide to display this button for everyone.
...
And thanks to all individual contributors who have made a donation via my liberapay page.`,description:"Plugin Credits",tags:[],title:"Credits",uri:"/peertube-plugin-livechat/zh-hant/credits/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:`Enabling the chat for you live streams 警告 Instance administrators can choose to disable or enable chat in specific cases. Information in this section are only true in the default case.
When you create or modify a Peertube live, there is a “plugin settings” tab:
In the “plugin settings” tab, there is a “使用聊天功能” checkbox. Just check or uncheck it to enable or disable the chat associated to your video.
提示 There can be other settings in this tab, depending on plugins installed on your Peertube instance.
...
Share the chat On top of the chat, there is a “分享聊天連結” button.
...
唯讀: you will only be able to read the chat, not write. This is useful to include the chat content in your live stream (see the OBS documentation). 使用當前主題顏色: if checked, your current theme colors will be added to the url, so that any user that opens the link will have the same color set. 產生 iframe 以將聊天嵌入到網站中: instead of an url, you will obtain an HTML snippet that you can add to your website to embed the chat. For more information on the “Dock” tab, check the OBS documentation.
...
The chat will be automatically recreated each time someone tries to join it as long as the video exists, and has the “使用聊天功能” feature activated.`,description:"Some basics about how to setup and use the chat for your live stream",tags:[],title:"Some basics",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/basics/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 資訊 This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
In the Peertube left menu, there is a “聊天室” entry:
This “聊天室” link takes you to a list of your channels. By clicking on a channel, you will then be able to setup some options for your channels:
...
提示 The video owner will be owner of the chat room. This means they can configure the room, delete it, promote other users as admins, …
提示 Starting with livechat v10.0.0, Peertube instance’s admins and moderators have no special rights on rooms by default. However, they have a special button available on top of the chat: “成為版主”. Clicking this button will give them owner access on the room.
...
Mute anonymous users 資訊 This feature comes with the livechat plugin version 10.2.0.
...
Anonymize moderation actions 資訊 This feature comes with the livechat plugin version 11.0.0.
...
You can choose to enable or disable this feature for new chatrooms on the channel configuration page.
Participant message history search 資訊 This feature comes with the livechat plugin version 11.0.0.
As a room admin or owner, you can search all messages sent by a given participant.
To do so, you have several ways:
using the “Search all messages” action in the dropdown menu besides participants in the sidebar using the “Search all messages” action in the dropdown menu besides chat messages 提示 To have more space and better readability, open the chat in full-page mode.
...
From there, you can also promote yourself as room moderator by using the “成為版主” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 資訊 This feature comes with the livechat plugin version 10.2.0.
...
資訊 Peertube instance’s admin can also set global terms & conditions. If so, these terms will be shown above your channel’s terms.
資訊 Anonymous users will only see the terms & conditions once they have chosen their nickname (in other words: once they are able to talk).
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 資訊 This feature comes with the livechat plugin version 10.3.0.
...
提示 You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 資訊 This feature comes with the livechat plugin version 10.1.0.
Channel emojis Streamers can add custom emojis to their channels.
On the channel configuration page, open the “頻道emojis” tab:
您可以為您的頻道組態自訂表情符號。 這些表情符號將在表情符號選擇器中提供。 使用者也可以透過其短名稱來使用它們（例如透過編寫“:shortname:”）。 You can use emojis in the chat using ":shortname:". The short name can start and/or end by a colon (:), and only contain alphanumerical characters, underscores and hyphens. It is strongly recommended to start them by a colon, so that users can use autocompletion (by typing ":" then press TAB). Import / Export On the channel configuration page, there are an “匯入” and an “匯出” button. The “匯出” button generates a file than you can then import on another channel.
...
提示 Anonymous viewers can only vote once they have choosen their nickname.
...
資訊 For viewers using XMPP clients or outdated livechat plugin versions, the banner will not be visible. But they will see the message in the chat and will be able to vote by sending messages with their choices.
When the poll ends, a new message will be sent in the chat, with the results.
資訊 The only way to get old polls results is to search for the poll end message in the chat. For now, polls results are not saved by any other means. So don’t forget to note polls results if you want to keep them.`,description:"You can create polls to ask viewers their opinion.",tags:[],title:"Polls",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/polls/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 資訊 This feature comes with the livechat plugin version 10.0.0.
...
提示 All modification are instantly visible in all your browser tabs, and for all room’s admins.
...
Sorting notes You can sort notes simply using drag & drop.`,description:"Plugin peertube-plugin-livechat moderation notes",tags:[],title:"Moderation notes",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/moderation_notes/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` 資訊 This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
The bot will reload instantly when you save the page.`,description:"Chat bot setup",tags:[],title:"Chat bot",uri:"/peertube-plugin-livechat/zh-hant/documentation/user/streamers/bot/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"類別",uri:"/peertube-plugin-livechat/zh-hant/categories/index.html"},{breadcrumb:"",content:`PeerTube plugin livechat 提示 You can use the language selector in the left menu to view this documentation in different languages. Some translations are missing or incomplete. In this case, you’ll see the English version of the text.
...
提示 You can use the searchbox in the left menu to quickly find specific documentation parts.
資訊 Before updating to a major release, please read the release notes and breaking changes list : CHANGELOG.
...
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/zh-hant/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"標籤",uri:"/peertube-plugin-livechat/zh-hant/tags/index.html"}]