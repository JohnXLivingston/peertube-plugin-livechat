...
You can even activate the chat for specific VOD videos. This is how the demo page works: it is not a live stream, but I have activated the chat specifically for this video.`,description:"Introduction",tags:[],title:"Introduction",uri:"/peertube-plugin-livechat/sq/intro/index.html"},{breadcrumb:"Peertube livechat > Contributing",content:` Tip This Code of Conduct is adapted from the Contributor Covenant, version 2.1, available at https://www.contributor-covenant.org/version/2/1/code_of_conduct.html. Translations are available at https://www.contributor-covenant.org/translations. Instances of abusive, harassing, or otherwise unacceptable behavior may be reported to the community leaders responsible for enforcement by mail at git.[at].john-livingston.fr.
...
The Peertube instance can configure external authentication providers (Mastodon accounts, Google accounts, …). In such case, you will see a “Bëni hyrjen duke përdorur një llogari të jashtme” button, that will open a dialog modal. In this dialog modal, there will be some buttons to connect using a remote account.
...
You can also change your nickname using the chat menu.`,description:"How to chat for stream viewers",tags:[],title:"For viewers",uri:"/peertube-plugin-livechat/sq/documentation/user/viewers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers > Chat bot",content:`You can configure some words that will be automatically moderated by the bot (messages containing such words will be instantly deleted). You can also add an optional reason that will be displayed at the place of deleted messages. Several examples are provided on the documentation page. You can fill several “Fjalë, ose shprehje të ndaluara” fields. When a user sends a message that match the configured criteria, the message will automatically be deleted.
...
Also moderate messages from moderators By default, moderator messages will not be deleted when containing forbidden words. By checking this option, messages from moderators will also be deleted. Arsye Reason to display besides deleted messages Komente You can add here some comments about this rule, to remember how and why you created it. These comments are purely indicative, and have no influence on the bot's behavior. `,description:"The bot can automatically moderate messages containing forbidden words.",tags:[],title:"Forbidden words",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/bot/forbidden_words/index.html"},{breadcrumb:"Peertube livechat > Documentation > Admin documentation",content:`This section describes the plugin settings page.
Terma & kushte kanali fjalosjeje Mund të formësoni një ndarje “terma & kushte” që do t’u shfaqet përdoruesve që hyjnë në dhomat tuaja. For more information on this feature, check the documentation for channel’s terms & conditions.
Info Changing this setting will restart the chat server, and all users will be disconnected for a short time.
Paraqit dhoma ekzistuese When pressing the «List rooms» button, all existing chatrooms will be listed. You can then find them and moderated them.
Federation Following settings concern the federation with other Peertube instances, and other fediverse softwares.
Mos shfaq fjalosje nga larg Duke i vënë shenjë këtij rregullimi, instanca juaj s’do të shfaqë kurrë fjalosje prej videosh të largëta. Mos boto hollësifjalosjeje By checking this setting, your instance will not publish chat information on the fediverse. Remote Peertube instances will not be aware that they are chat rooms associated to your videos.
...
Chat behaviour Lloj dhome Këtu mund të zgjidhni të keni dhoma më vete për çdo video, ose t’i gruponi sipas kanalesh. Hape vetvetiu fjalosjen Kur shihet një video, dritarja e fjalosjes do të hapet vetvetiu. Shfaq butonin “hape në dritare të re” Do të ketë një buton për hapje dritareje fjalosjeje në një dritare të re. Shfaq butonin “Ndani me të tjerët lidhje fjalosjeje” This feature enables a «share chat link» modal. With this modal, you can generate URLs to join the chat. The chat can be customized (readonly mode, use the current theme, …).
...
Përdoruesit mund të aktivizojnë fjalosje për transmetimet e tyre të drejtpërdrejta Në iu vëntë shenjë, krejt videot e transmetuara drejtpërdrejt do të kenë një kutizë te vetitë e tyre, për aktivizim fjalosjeje. The video owner will be able to activate web chats.
Aktivizo fjalosje për krejt të drejtpërdrejtat Në iu vëntë shenjë, fjalosja do të jetë e aktivizuar për krejt transmetimet e drejtpërdrejta. Aktivizo fjalosje për krejt transmetimet jo të drejtpërdrejta Aktivizo fjalosje për krejt transmetimet jo të drejtpërdrejta Aktivizo fjalosje për këto video Aktivizo fjalosje për këto video Fshihe fjalosjen për përdorues anonimë If checked, anonymous Peertube users won't see the chat. This feature is still experimental. If you enabled it, it is highly recommended to also check "Don't publish chat information". Otherwise, some third party tools could try to open the chat, and have unpredictable behaviours. Note: for now this feature simply hide the chat. In a future release, the chat will be replaced by a message saying «please log in to […]». See v5.7.0 Release Notes for more information.
...
Theming Grup avatarësh You can choose from several different sets the default avatars that will be used for chat users.
...
In last resort, it will use your Peertube public URI. So, any API Call will go throught your Nginx server. This could fail in some case: for example if you are in a Docker container, where the public hostname does not resolve to the correct IP. In such case, try changing the “URL Peertube për thirrje API” settings, by setting http://127.0.0.1:9000 (assuming 9000 is the port on which Peertube listen, ask your instance administrators if you don’t know).
Regjistro ç’bëhet në dhomë, si parazgjedhje If checked, room contents will be saved by default. Any user who joins a room will see what was written before they joins.
Please note that it is always possible to enable/disable the content archiving for a specific room, by editing its properties. Skadim regjistrash dhome Këtu mund të zgjidhni se për sa kohë mbahet lënda e dhomës së fjalosjes nga shërbyesi. Vlera mund të jetë: 60: lënda do të ruhet për 60 sekonda. 60-n mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1d: lënda do të ruhet për 1 ditë. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1w: lënda do të ruhet për 1 javë. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1m: lënda do të ruhet për 1 muaj. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1y: lënda do të ruhet për 1 vit. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. never: lënda nuk skadon kurrë dhe do të mbahet përgjithnjë. Aktivizoni lidhje te dhoma duke përdorur llogari të jashtme XMPP By enabling this option, it will be possible to connect to rooms using external XMPP accounts and XMPP clients.
Warning, enabling this option can request extra server and DNS configuration. Please refer to the documentation: Enable external XMPP account connections. Portë Prosody shërbyes te shërbyes The port that will be used for XMPP s2s (server to server) connections.
You should use the standard 5269 port. Otherwise you should setup a specific DNS record . Ndërfaqe rrjeti shërbyes te shërbyes The network interfaces to listen on for server to server connections.
...
Examples: *, :: * 127.0.0.1, ::1 172.18.0.42 Dosje dëshmish If this field is empty, the plugin will generate and use self-signed certificates.
If you want to use other certificates, just specify here the folder where Prosody can find them. Note: the "peertube" user must have read access to this folder. Aktivizo lidhje klient te shërbyes Enable XMPP clients to connect to the built-in Prosody server.
...
Klient Prosody te portë shërbyesi The port that will be used by the c2s module of the built-in Prosody server.
...
Note: this might change in a near future, as it is planned to add a feature to activate external connections. Ndërfaqe rrjeti klient te shërbyes The network interfaces to listen on for client to server connections.
...
This feature can, for example, be used to connect some bots to the chatting rooms. Portë përbërësish Prosody të jashtëm The port that will be used by XMPP components to connect to the Prosody server.
Change it if this port is already in use on your server.
...
In such case, try changing the “URL Peertube për thirrje API” settings, by setting http://127.0.0.1:9000 (assuming 9000 is the port on which Peertube listen, ask your instance administrators if you don’t know).
...
If you can’t fix this immediatly, you can disable Websocket by unchecking “Çaktivizo Websocket-in” in the plugin setting page. In such case, you should also check “Mos boto hollësifjalosjeje”, as chat federation won’t work without Websocket.`,description:"Some classic mistakes and workarounds.",tags:[],title:"Troubleshooting",uri:"/peertube-plugin-livechat/sq/documentation/installation/troubleshooting/index.html"},{breadcrumb:"Peertube livechat > Documentation",content:` For viewersHow to chat for stream viewers
OBSDocumentation to stream the chat content using OBS.
XMPP ClientsConnect to chat using a XMPP client
For streamersHow to setup the chat for your live stream
Some basicsSome basics about how to setup and use the chat for your live stream
...
To enable this feature, first you have to create a client on your provider side (check the related documentation for enabling OpenID Connect). Then go to the plugin settings, and enable “Përdorni një shërbim OpenID Connect”.
...
Etiketë për butonin e lidhjes Kjo etiketë do t’ju shfaqet përdoruesve, si etiketa e butonit për kryerje mirëfilltësimi me këtë shërbim OIDC. This is the button label in the following screenshot:
...
ID Klienti Your application Client ID.
E fshehtë klienti You application Client secret.
...
TroubleshootingSome classic mistakes and workarounds.
Known issues: CPU CompatibilityFor now, the plugin only works out of the box for x86_64 and arm64 CPU architecture. Here are some instructions for other CPU architectures.
Upgrade from version older than 6.0.0Important notes when upgrading for an older version.`,description:"Plugin peertube-plugin-livechat installation guide",tags:[],title:"Installation guide",uri:"/peertube-plugin-livechat/sq/documentation/installation/index.html"},{breadcrumb:"Peertube livechat > Documentation > Installation guide",content:`The Prosody AppImage included in the plugin will only work on x86_64 and arm64 CPU. It is not compatible with other CPU architectures.
...
To do so, just use the “Ndajeni lidhjen e fjalosjes me të tjerët” feature, and open the “Dock” tab. From there, you can create a new token using the “+” button.
...
Internal connections only (basic) You will need to enable Aktivizo lidhje klient te shërbyes in the livechat plugin settings.
...
Appendix F: Requirements Conformance The following requirements keywords as used in this document are to be interpreted as described in RFC 2119: “MUST”, “SHALL”, “REQUIRED”; “MUST NOT”, “SHALL NOT”; “SHOULD”, “RECOMMENDED”; “SHOULD NOT”, “NOT RECOMMENDED”; “MAY”, “OPTIONAL”.`,description:"MUC Slow mode XEP",tags:[],title:"MUC Slow mode",uri:"/peertube-plugin-livechat/sq/technical/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:`This chat plugin relies on the XMPP protocol (also known as Jabber). It is therefore possible to connect to the chats using XMPP client software. This can be useful for example to facilitate moderation operations.
...
And thanks to all individual contributors who have made a donation via my liberapay page.`,description:"Plugin Credits",tags:[],title:"Credits",uri:"/peertube-plugin-livechat/sq/credits/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:`Enabling the chat for you live streams Warning Instance administrators can choose to disable or enable chat in specific cases. Information in this section are only true in the default case.
When you create or modify a Peertube live, there is a “plugin settings” tab:
In the “plugin settings” tab, there is a “Përdorni fjalosjen” checkbox. Just check or uncheck it to enable or disable the chat associated to your video.
...
Share the chat On top of the chat, there is a “Ndajeni lidhjen e fjalosjes me të tjerët” button.
This button opens a popup, where you can obtain an url to join the chat. This url can be shared.
The “Trupëzojeni” tab provide some links to embed the chat in websites, or in your live stream.
You can customize some options:
Vetëm për lexim: you will only be able to read the chat, not write. This is useful to include the chat content in your live stream (see the OBS documentation). Përdor ngjyra të temës së tanishme: if checked, your current theme colors will be added to the url, so that any user that opens the link will have the same color set. Generate an iframe to embed the chat in a website: instead of an url, you will obtain an HTML snippet that you can add to your website to embed the chat. For more information on the “Dock” tab, check the OBS documentation.
...
The “Ndajeni lidhjen e fjalosjes me të tjerët” popup can also contain a “Lidhu duke përdorur XMPP” tab. This will only be available if your instance’s administators have enabled an correctly configured this option. Using this option, you can provide a link to join the chat using any XMPP client software. Using such softwares can for example facilitate moderation actions.
...
The chat will be automatically recreated each time someone tries to join it as long as the video exists, and has the “Përdorni fjalosjen” feature activated.`,description:"Some basics about how to setup and use the chat for your live stream",tags:[],title:"Some basics",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/basics/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
Terma & kushte kanali fjalosjeje Heshto përdorues anonimë default value The slow mode The chat bot Custom emojis More features to come… `,description:"Peertube channel chatrooms configuration",tags:[],title:"Channel configuration",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/channel/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation",content:` Some basicsSome basics about how to setup and use the chat for your live stream
Channel configurationPeertube channel chatrooms configuration
ModerationPlugin peertube-plugin-livechat advanced moderation features
Terms & conditionsConfigure channel's chat terms & conditions
Slow modePlugin peertube-plugin-livechat slow mode
...
Tip Starting with livechat v10.0.0, Peertube instance’s admins and moderators have no special rights on rooms by default. However, they have a special button available on top of the chat: “Bëhuni moderator”. Clicking this button will give them owner access on the room.
...
Heshto përdorues anonimë Info This feature comes with the livechat plugin version 10.2.0.
You can prevent anonymous users to send messages. In such case, only registered users will be able to talk in the chat.
To enable or disable this feature, use the chat dropdown menu, open the “configure” menu. In the form, you will find a “Heshto përdorues anonimë” checkbox.
Anonymous users won’t have the message field, and will see following prompt: “Mesazhe mund të dërgojnë vetëm përdorues të regjistruar.”
...
From there, you can also promote yourself as room moderator by using the “Bëhuni moderator” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.2.0.
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.3.0.
...
Moderation delay option On the channel configuration page, you can set the “Vonesë moderimi” option:
This value will apply as a default value for all your channel’s chatrooms.
Setting the value to 0 will disable the feature.
...
Tip You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.1.0.
Channel emojis Streamers can add custom emojis to their channels.
On the channel configuration page, open the “Emoxhi kanali” tab:
Mund të formësoni emoxhi vetjake për kanalin tuaj. Këto emoxhi do të jenë të përdorshëm që nga zgjedhësi i tyre. Përdoruesit mund t’i përdoren edhe përmes emrit të shkurtër përkatës (për shembull, duke shkruar ":emrineshkurtër:"). You can use emojis in the chat using ":shortname:". The short name can start and/or end by a colon (:), and only contain alphanumerical characters, underscores and hyphens. It is strongly recommended to start them by a colon, so that users can use autocompletion (by typing ":" then press TAB). Import / Export On the channel configuration page, there are an “Importoje” and an “Eksportoje” button. The “Eksportoje” button generates a file than you can then import on another channel.
...
[ { "sn": ":short_name:", "url": "https://example.com/image.png" } ] The sn attribute is the short name code. The url attribute can be any image url than your browser can access, or a Data URL representing the file you want to import.`,description:"Plugin peertube-plugin-livechat custom emojis",tags:[],title:"Custom emojis",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/emojis/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.2.0.
Create a poll You can create a new poll by using the “Krijoni një pyetësor të ri” action in the chat top menu:
...
“Pyetje”: the question to ask to you viewers “Kohëzgjatje pyetësori (në minuta)”: the duration for which viewers can vote “Përfundime anonime”: if checked, votes won’t be publicly visible in the chat “Choice N”: choices that will be presented to viewers You must at least fill the two first choices fields.
...
When you promote someone as room admin or owner, they gets instant access to the “Krijoni një pyetësor të ri” action.
...
Every user that is not muted can vote. This means that you can prevent anonymous users to vote by using the “Heshto përdorues anonimë” feature.
...
If “Përfundime anonime” is checked, votes won’t be shown to other users. If unchecked, votes will be publicly visible as you will see message like “!1” in the chat.
...
share some notes between moderators take notes about participants that were kicked or caused troubles … Using the Moderator Notes Application Opening the Moderator Notes Application To open the Moderator Notes Application, there is a “Shënime moderimi” button in the top chat menu:
...
Sorting notes You can sort notes simply using drag & drop.`,description:"Plugin peertube-plugin-livechat moderation notes",tags:[],title:"Moderation notes",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/moderation_notes/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 8.0.0, and can be disabled by your instance’s admins.
...
CreditsPlugin Credits`,description:"Peertube plugin livechat documentation",tags:[],title:"Peertube livechat",uri:"/peertube-plugin-livechat/sq/index.html"},{breadcrumb:"Peertube livechat",content:"",description:"",tags:[],title:"Tags",uri:"/peertube-plugin-livechat/sq/tags/index.html"}]