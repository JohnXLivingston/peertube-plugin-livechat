...
Mos shfaq fjalosje nga larg Duke i vënë shenjë këtij rregullimi, instanca juaj s’do të shfaqë kurrë fjalosje prej videosh të largëta.
Mos boto hollësifjalosjeje By checking this setting, your instance will not publish chat information on the fediverse. Remote Peertube instances will not be aware that they are chat rooms associated to your videos.
...
Shfaq butonin “Ndani me të tjerët lidhje fjalosjeje” This feature enables a «share chat link» modal. With this modal, you can generate URLs to join the chat. The chat can be customized (readonly mode, use the current theme, …).
...
Përdoruesit mund të aktivizojnë fjalosje për transmetimet e tyre të drejtpërdrejta Në iu vëntë shenjë, krejt videot e transmetuara drejtpërdrejt do të kenë një kutizë te vetitë e tyre, për aktivizim fjalosjeje.
The video owner will be able to activate web chats.
Aktivizo fjalosje për krejt të drejtpërdrejtat Në iu vëntë shenjë, fjalosja do të jetë e aktivizuar për krejt transmetimet e drejtpërdrejta.
Aktivizo fjalosje për krejt transmetimet jo të drejtpërdrejta Në iu vëntë shenjë, fjalosja do të jetë e aktivizuar për krejt videot që s’janë transmetimet e drejtpërdrejta.
Aktivizo fjalosje për këto video Videos UUIDs for which we want a web chat (short UUID or UUIDv4). Can be non-live videos. One per line. You can add comments: everything after the # character will be stripped off, and empty lines ignored.
Don’t add private videos, the UUIDs will be sent to the frontend.
Fshihe fjalosjen për përdorues anonimë If checked, anonymous Peertube users won’t see the chat. This feature is still experimental. If you enabled it, it is highly recommended to also check “Don’t publish chat information”. Otherwise, some third party tools could try to open the chat, and have unpredictable behaviours.
...
Peertube theme: this is a special theme, made especially for peertube’s integration. Default ConverseJS theme: this is the default ConverseJS theme. ConverseJS cyberpunk theme: this is a theme provided by ConverseJS. Pikasje e vetvetishme ngjyrash Try to autodetect colors from user’s current theme.
...
Chat server advanced settings Përdor Prosody sistemi The plugin comes with an AppImage that is used to run the Prosody XMPP server. If this AppImage is not working, you can fallback to the Prosody that is packaged for your server. Just install the prosody package.
...
Portë Prosody The port that will be used by the Prosody server.
...
URL Peertube për thirrje API Please let this setting empty if you don’t know what you are doing.
...
Skadim regjistrash dhome Këtu mund të zgjidhni se për sa kohë mbahet lënda e dhomës së fjalosjes nga shërbyesi. Vlera mund të jetë:
60: lënda do të ruhet për 60 sekonda. 60-n mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1d: lënda do të ruhet për 1 ditë. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1w: lënda do të ruhet për 1 javë. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1m: lënda do të ruhet për 1 muaj. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. 1y: lënda do të ruhet për 1 vit. 1-shin mund ta zëvendësoni me çfarëdo vlere numër të plotë. never: lënda nuk skadon kurrë dhe do të mbahet përgjithnjë. Aktivizoni lidhje te dhoma duke përdorur llogari të jashtme XMPP By enabling this option, it will be possible to connect to rooms using external XMPP accounts and XMPP clients.
...
Ndërfaqe rrjeti shërbyes te shërbyes The network interfaces to listen on for server to server connections.
...
Examples:
*, :: * 127.0.0.1, ::1 172.18.0.42 Dosje dëshmish If this field is empty, the plugin will generate and use self-signed certificates.
...
Aktivizo lidhje klient te shërbyes Enable XMPP clients to connect to the built-in Prosody server.
...
Note: this might change in a near future, as it is planned to add a feature to activate external connections.
Ndërfaqe rrjeti klient te shërbyes The network interfaces to listen on for client to server connections.
...
Portë përbërësish Prosody të jashtëm The port that will be used by XMPP components to connect to the Prosody server.
Change it if this port is already in use on your server.
...
You can use «*» to listen on all IPv4 interfaces, and «::» for all IPv6.
Examples:
*, :: * 127.0.0.1, ::1 172.18.0.42 Përbërës të jashtëm The external components to declare:
...
Etiketë për butonin e lidhjes Kjo etiketë do t’ju shfaqet përdoruesve, si etiketa e butonit për kryerje mirëfilltësimi me këtë shërbim OIDC.
...
You can fill several “Fjalë, ose shprehje të ndaluara” lines. When a user sends a message that match the configured criteria, the message will automatically be deleted.
Fjalë, ose shprehje të ndaluara Here you can configure several words, group of words, or “regular expressions”.
...
Konsideroji shprehje të rregullta Warning This feature can be disabled by the instance’s adminitrators.
By checking this option, each line of the “Fjalë, ose shprehje të ndaluara” field will be considered as a regular expression.
...
Then, choose “Lidhu duke përdorur XMPP”:
...
Forbidden wordsThe bot can automatically moderate messages containing forbidden words.
TimersThe bot can send periodically some messages.
CommandsThe bot can respond to several commands.`,description:"How to setup the chat for your live stream",tags:[],title:"For streamers",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 12.0.0. Room owners and administrators can send special announcements in the chat.
These messages will be more visible than standard messages.
To send announcements, owners and administrators will have a “Lloj mesazhi” selector on the top of the message field:
...
Heshto përdorues anonimë Info This feature comes with the livechat plugin version 10.2.0. You can prevent anonymous users to send messages. In such case, only registered users will be able to talk in the chat.
...
From there, you can also promote yourself as room moderator by using the “Bëhuni moderator” button on the right.`,description:"Plugin peertube-plugin-livechat advanced moderation features",tags:[],title:"Moderation",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/moderation/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.2.0. Configuration You can add terms & conditions to your channel. These terms will be shown to all users joining the chat.
...
This limitation does not apply to moderators.`,description:"Plugin peertube-plugin-livechat slow mode",tags:[],title:"Slow mode",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/slow_mode/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.3.0. Introduction As a streamer, you can choose to delay messages in the chat, to let some time to moderators to delete messages before they can even be read by other participants.
...
Tip You can combine a short moderation delay (1 second for example) with the moderation bot to delete messages containing swear words before any non-moderator user will see them.
In the chat As a moderator, you will see the remaining time (in seconds) before the message is broadcasted, just besides the message datetime.`,description:"Plugin peertube-plugin-livechat moderation delay",tags:[],title:"Moderation delay",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/moderation_delay/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.1.0. Channel emojis Streamers can add custom emojis to their channels.
On the channel configuration page, open the “Emoxhi kanali” tab:
Mund të formësoni emoxhi vetjake për kanalin tuaj. Këto emoxhi do të jenë të përdorshëm që nga zgjedhësi i tyre. Përdoruesit mund t’i përdoren edhe përmes emrit të shkurtër përkatës (për shembull, duke shkruar “:emrineshkurtër:”).
...
Import / Export On the channel configuration page, there are an “Importoje” and an “Eksportoje” button. The “Eksportoje” button generates a file than you can then import on another channel.
...
[ { "sn": ":short_name:", "url": "https://example.com/image.png" } ] The sn attribute is the short name code. The url attribute can be any image url than your browser can access, or a Data URL representing the file you want to import.`,description:"Plugin peertube-plugin-livechat custom emojis",tags:[],title:"Custom emojis",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/emojis/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 12.0.0. Emojis only mode You can enable an “Emojy only mode” in your chatrooms. When this mode is enabled, participants can only send emojis (standard, or channel custom emojis). Moderators are not affected by this limitation.
...
If you want to enable it for all your chatrooms at once, open the channel emojis configuration page, and use the “Enable the emoji only mode on all channel’s chatrooms” button.`,description:"Plugin peertube-plugin-livechat emojis only mode",tags:[],title:"Emojis only mode",uri:"/peertube-plugin-livechat/sq/documentation/user/streamers/emojis_only/index.html"},{breadcrumb:"Peertube livechat > Documentation > User documentation > For streamers",content:` Info This feature comes with the livechat plugin version 10.2.0. Create a poll You can create a new poll by using the “Krijoni një pyetësor të ri” action in the chat top menu:
